...
/*-----------------------------------------------------------*/
	...
	static char *prvWriteNameToBuffer( char *pcBuffer, const char *pcTaskName )
	{
	BaseType_t x = strlen( pcTaskName );

		/* Start by copying the entire string. */
		if (x >= ( configMAX_TASK_NAME_LEN - 1 ))
            x = configMAX_TASK_NAME_LEN - 1;

		strncpy( pcBuffer, pcTaskName, x);
        pcBuffer[x] = 0x00;
	...
	char cStatus;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			for( x = 0; x < uxArraySize; x++ )
			{
				...
				{
                    char tempName[configMAX_TASK_NAME_LEN] ;

                    prvWriteNameToBuffer( &tempName[0], pxTaskStatusArray[ x ].pcTaskName );
                    printf("%s\t%c\t%u\t%u\t%u\r\n", &tempName[0], cStatus, ( unsigned int ) pxTaskStatusArray[ x ].uxCurrentPriority, ( unsigned int ) pxTaskStatusArray[ x ].usStackHighWaterMark, ( unsigned int ) pxTaskStatusArray[ x ].xTaskNumber );
                }
			}
					...
					if( ulStatsAsPercentage > 0UL )
					{
						#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
						{
							if (pcWriteBuffer != NULL)
                                ...
                                printf("%s\t%lu\t\t%lu%%\r\n",  &tempName[0], pxTaskStatusArray[ x ].ulRunTimeCounter, ulStatsAsPercentage );
						}
						#else
						{
							...
							if (pcWriteBuffer != NULL)
                                ...
                                printf("%s\t%u\t\t%u%%\r\n", &tempName[0], ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage );
						}
						#endif
					}
					else
					{
						...
						#ifdef portLU_PRINTF_SPECIFIER_REQUIRED
						{
                            if (pcWriteBuffer != NULL)
                                ...
                                printf( "%s\t%lu\t\t<1%%\r\n", &tempName[0], pxTaskStatusArray[ x ].ulRunTimeCounter );
						}
						#else
						{
							/* sizeof( int ) == sizeof( long ) so a smaller
							...
							if (pcWriteBuffer != NULL)
                                ...
                                printf("%s\t%u\t\t<1%%\r\n", &tempName[0], ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter );
						}
						#endif
					}
                    if (pcWriteBuffer != NULL)
