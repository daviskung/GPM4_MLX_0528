...
xQueueHandle midi_wq;
xQueueHandle midi_fsq;
...
#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
      midi_wq = xQueueCreate(AUDIO_WRITE_Q_SIZE, sizeof(INT32U));
      midi_fsq = xQueueCreate(AUDIO_FS_Q_SIZE, sizeof(INT32U));
#endif
...
static void midi_start(STAudioTaskPara *pAudioTaskPara)
{
	...
	if (ret == AUDIO_ERR_NONE) {
		...
		drv_l1_dac_enable_set(TRUE); /* enable dac */
...
static void midi_spu_restart(void)
{
	...
	for (count=0;count<MAX_DAC_BUFFERS;count++) {
		...
		OSQPost(midi_wq, (void *) count);
         #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
                xQueueSend(midi_wq, (void *)&count, portMAX_DELAY);
         #endif
	}
	midi_send_next_frame_q();
}
	...
	switch(midi_context_p->audio_format)
	{
		case AUDIO_TYPE_MIDI:
				if(midi_ctrl.file_handle >= 0)
					fs_close(midi_ctrl.file_handle);
                #if _DRV_L1_SPU == 1
				SPU_MIDI_Stop();
				SPU_Free_Midi();
				#endif
