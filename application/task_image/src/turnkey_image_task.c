...
osMessageQId image_task_fs_queue_a = NULL;
osMessageQId image_task_fs_queue_b = NULL;
#endif
...
void image_task_init(void)
{
	...
	#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
	if (!image_task_fs_queue_a) {
        osMessageQDef_t fs_fifo_buf_a_q = {C_IMAGE_DECODE_FILE_QUEUE_SIZE,sizeof(INT32U), 0};
		image_task_fs_queue_a = osMessageCreate(&fs_fifo_buf_a_q, NULL);
	}
	if (!image_task_fs_queue_b) {
        osMessageQDef_t fs_fifo_buf_b_q = {C_IMAGE_DECODE_FILE_QUEUE_SIZE,sizeof(INT32U), 0};
		image_task_fs_queue_b = osMessageCreate(&fs_fifo_buf_b_q, NULL);
	}
    #endif
}
