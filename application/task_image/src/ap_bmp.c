...
INT32S bmp_file_parse_header(IMAGE_HEADER_PARSE_STRUCT *parser)
{
	...
	do {
		...
		#if (_OPERATING_SYSTEM == _OS_UCOS2)
		len = (INT32S) OSQPend(image_task_fs_queue_a, 2000, &error);
...
INT32S bmp_scaler_set_parameters(INT32U fifo)
{
	...
	scale.input_width = bmp_extend_width;
	scale.input_height = bmp_valid_height;
	scale.input_visible_width = bmp_valid_width;
	scale.input_visible_height = bmp_valid_height;
	scale.input_x_offset = 0;
	scale.input_y_offset = 0;

    scale.input_y_addr = (INT32U)bmp_decode_output;
    scale.input_u_addr = 0;
    scale.input_v_addr = 0;

    scale.input_b_y_addr = (INT32U)(bmp_decode_output+(bmp_extend_width*C_BMP_SCALE_FIFO_LINE_NUM*2));
	...
	scale.output_format = bmp_output_format;
	scale.output_width = bmp_output_buffer_width;
	scale.output_height = bmp_output_buffer_height;
	scale.output_buf_width = bmp_output_buffer_width;
	scale.output_buf_height = bmp_output_buffer_height;
	scale.output_x_offset = 0;
    scale.output_y_addr = (INT32U)bmp_scaler_out_buffer;
    scale.output_u_addr = 0;
    scale.output_v_addr = 0;

    scale.callback = 0;
	scale.fifo_mode = fifo;
	scale.digizoom_m = 10;
	scale.digizoom_n = 10;

	gp_memset((INT8S *)&para, 0x00, sizeof(para));
	para.boundary_mode = 1;
	para.boundary_color = bmp_out_of_boundary_color;
	...
	if (bmp_output_buffer_pointer) {
 	...
 	drv_l1_scaler_output_addr_set(C_USE_SCALER_NUMBER,bmp_scaler_out_buffer, (INT32U)NULL, (INT32U)NULL);
...
INT32S bmp_file_decode_and_scale(IMAGE_DECODE_STRUCT *img_decode_struct)
{
	...
	INT8U scaler_done;
	TK_FILE_SERVICE_STRUCT fs_cmd_a={0}, fs_cmd_b={0};
	INT32S read_cnt;
	INT8U error, fifo_a;
	...
	bmp_decode_init();
   	do {
		...
		#if (_OPERATING_SYSTEM == _OS_UCOS2)
		read_cnt = (INT32S) OSQPend(image_task_fs_queue_a, 2000, &error);
	...
	if ((INT32U) bmp_input_data_addr == (fs_cmd_a.buf_addr + read_cnt)) {
		...
		fifo_a ^= 0x1;
		#if (_OPERATING_SYSTEM == _OS_UCOS2)
		read_cnt = (INT32S) OSQPend(image_task_fs_queue_b, 2000, &error);
		if ((error!=OS_NO_ERR) || (read_cnt<=0)) {
		#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
		...
		if((result.status != osEventMessage) || (result.value.signals <0)) {
			...
			img_decode_struct->decode_status = STATUS_FAIL;
			#if (_OPERATING_SYSTEM == _OS_UCOS2)
			OSQPend(image_task_fs_queue_a, 2000, &error);
			#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
			osMessageGet(image_task_fs_queue_a, 2000);
			#endif
		  	gp_free((void *) fs_cmd_a.buf_addr);
    		gp_free((void *) fs_cmd_b.buf_addr);
    	  	gp_free((void *) bmp_decode_output);
	...
	if (bmp_decode_on_the_fly_start(bmp_input_data_addr, fly_len)) {
		...
		if (fifo_a) {
            ...
            #if (_OPERATING_SYSTEM == _OS_UCOS2)
			OSQPend(image_task_fs_queue_b, 2000, &error);
			#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
			osMessageGet(image_task_fs_queue_b, 2000);
			#endif
		}
	  	gp_free((void *) fs_cmd_a.buf_addr);
		gp_free((void *) fs_cmd_b.buf_addr);
		...
		if (bmp_status & C_BMP_STATUS_DECODE_DONE) {
		  	...
		  	while (!scaler_done) {
                ...
                if(scaler_status == C_SCALER_STATUS_STOP) {
                    ...
                    scaler_status = drv_l2_scaler_retrigger(C_USE_SCALER_NUMBER, &scale);
                } else if(scaler_status & C_SCALER_STATUS_TIMEOUT) {
                    DBG_PRINT("Scale1Timeout.\r\n");
                    break;
                } else if(scaler_status & C_SCALER_STATUS_INIT_ERR) {
                    DBG_PRINT("Scale1InitErr.\r\n");
                    break;
                } else {
                    DBG_PRINT("Scale1StatusErr = 0x%x\r\n", scaler_status);
                    break;
                }
