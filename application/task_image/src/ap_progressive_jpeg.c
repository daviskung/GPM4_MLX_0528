...
#if C_SCALER_L2_EN == 1
static ScalerFormat_t	prog_jpeg_src={0};
static ScalerPara_t		prog_jpeg_para={0};
#endif
...
INT32S progressive_jpeg_read_data(void)
{
	...
	if (pjpeg_source_type == TK_IMAGE_SOURCE_TYPE_FILE) {				// Read and decode JPEG from file system
		...
		if (pjpeg_write_index == 0) {
			...
			read_count = (INT32S) OSQPend(pjpeg_fs_cmd_a.result_queue, 2000, &error);
		} else {
			msgQSend(fs_msg_q_id, MSG_FILESRV_FS_READ, (void *) &pjpeg_fs_cmd_b, sizeof(TK_FILE_SERVICE_STRUCT), MSG_PRI_NORMAL);
			read_count = (INT32S) OSQPend(pjpeg_fs_cmd_b.result_queue, 2000, &error);
		}
		...
		if (pjpeg_write_index == 0) {
			msgQSend(fs_msg_q_id, MSG_FILESRV_FS_READ, (void *) &pjpeg_fs_cmd_a, sizeof(TK_FILE_SERVICE_STRUCT), MSG_PRI_NORMAL);
			//read_count = (INT32S) OSQPend(pjpeg_fs_cmd_a.result_queue, 2000, &error);
			result = osMessageGet(pjpeg_fs_cmd_a.result_queue, 2000);
			read_count = result.value.signals;
		} else {
			msgQSend(fs_msg_q_id, MSG_FILESRV_FS_READ, (void *) &pjpeg_fs_cmd_b, sizeof(TK_FILE_SERVICE_STRUCT), MSG_PRI_NORMAL);
			//read_count = (INT32S) OSQPend(pjpeg_fs_cmd_b.result_queue, 2000, &error);
			result = osMessageGet(pjpeg_fs_cmd_b.result_queue, 2000);
			read_count = result.value.signals;
		}
		if ((result.status!=osEventMessage) || (result.value.signals<0)) {
			DBG_PRINT("Failed to read progressive file\r\n");
...
INT32S jpeg_decode_progressive(IMAGE_DECODE_STRUCT *img_decode_struct)
{
	...
	INT32S width_in_block[3]={0}, height_in_block[3]={0};//, YCbCr_Img_Size[3]={0};
	INT32S YCbCr_Size[3];
	INT32U hv[3], v[3];
	INT16U hsize, vsize;
	INT32U scaler_mode=0, coef_size=0, mode=0, pjpg_mode=0;
	INT32U cnt;
	INT16U *p_quant_tbl;
	...
	drv_l1_jpeg_quant_luminance_set(0, 64, p_quant_tbl);
	p_quant_tbl = progressive_jpeg_get_quant_tbl(pjpeg_working_memory, 1);
	drv_l1_jpeg_quant_chrominance_set(0, 64, p_quant_tbl);
	...
	while (1) {
...
#else
		{
			...
			// Hardware IDCT
			drv_l1_jpeg_init();
			drv_l1_jpeg_fifo_line_set(C_JPG_FIFO_DISABLE);

			// Set hw for progressive jpeg
			// DRV JPEG
			drv_l1_jpeg_progressive_mode_enable();  // Progressive mode enable & DCT coefficients are 2-byte accuracy

			// Set output addr: Y, Cb, Cr
			drv_l1_jpeg_yuv_addr_set(p_Y, p_Cb, p_Cr);

			// Set image size & YUV mode
			drv_l1_jpeg_image_size_set(img_w, img_h);
			drv_l1_jpeg_yuv_sampling_mode_set(mode);

			// Set input addr & size
			drv_l1_jpeg_vlc_addr_set((INT32U)coef_mem);
			drv_l1_jpeg_vlc_maximum_length_set(dct_size);

			drv_l1_jpeg_decompression_start(NULL);
			#if 1
			jpeg_status = drv_l1_jpeg_status_wait_idle(1);
			#else
			jpeg_status = drv_l1_jpeg_status_polling(1);
			#endif
			drv_l1_jpeg_stop();
  ...
  //#ifdef _DPF_PROJECT
...
#if C_SCALER_L2_EN==1
	prog_jpeg_src.input_format = scaler_mode;
	prog_jpeg_src.input_width = (width_in_block[0]+padding_blocks_h[0])<<3;
	prog_jpeg_src.input_height= (height_in_block[0]+padding_blocks_v[0])<<3;
	prog_jpeg_src.input_visible_width = hsize;
	prog_jpeg_src.input_visible_height = vsize;
	prog_jpeg_src.input_x_offset = 0;
	prog_jpeg_src.input_y_offset = 0;
	prog_jpeg_src.input_y_addr = (INT32U) YCbCr_Img[0];
	prog_jpeg_src.input_u_addr = (INT32U) YCbCr_Img[1];
	prog_jpeg_src.input_v_addr = (INT32U) YCbCr_Img[2];
	prog_jpeg_src.scale_mode = C_SCALER_BY_RATIO;

	prog_jpeg_src.output_format = img_decode_struct->output_format;
	prog_jpeg_src.output_y_addr = img_decode_struct->output_buffer_pointer;
	prog_jpeg_src.output_u_addr = 0;
	prog_jpeg_src.output_v_addr = 0;
	prog_jpeg_src.output_width	= img_decode_struct->output_image_width;
	prog_jpeg_src.output_height	= img_decode_struct->output_image_height;
	prog_jpeg_src.output_buf_width = img_decode_struct->output_buffer_width;
	prog_jpeg_src.output_buf_height= img_decode_struct->output_buffer_height;
	prog_jpeg_src.fifo_mode = C_SCALER_CTRL_FIFO_DISABLE;
	prog_jpeg_para.boundary_mode = 1;
	prog_jpeg_para.boundary_color = img_decode_struct->out_of_boundary_color;
	...
	while (!scaler_done) {
  		...
  		if (scaler_status == C_SCALER_STATUS_STOP) {
			...
			scaler_status = drv_l2_scaler_trigger(C_USE_SCALER_NUMBER, 1, &prog_jpeg_src, &prog_jpeg_para);
...
#else
	drv_l1_scaler_input_pixels_set(C_USE_SCALER_NUMBER,(width_in_block[0]+padding_blocks_h[0])<<3, (height_in_block[0]+padding_blocks_v[0])<<3);
	drv_l1_scaler_input_visible_pixels_set(C_USE_SCALER_NUMBER,hsize, vsize);
	//scaler_input_addr_set((INT32U) YCbCr_Img[0], (INT32U) YCbCr_Img[1], (INT32U) YCbCr_Img[2]);
	drv_l1_scaler_input_A_addr_set(C_USE_SCALER_NUMBER,(INT32U) YCbCr_Img[0], (INT32U) YCbCr_Img[1], (INT32U) YCbCr_Img[2]);
	drv_l1_scaler_input_format_set(C_USE_SCALER_NUMBER,scaler_mode);

	drv_l1_scaler_output_pixels_set(C_USE_SCALER_NUMBER,(hsize<<16)/img_decode_struct->output_image_width, (vsize<<16)/img_decode_struct->output_image_height, img_decode_struct->output_buffer_width, img_decode_struct->output_buffer_height);
	drv_l1_scaler_output_addr_set(C_USE_SCALER_NUMBER,img_decode_struct->output_buffer_pointer, (INT32U)NULL, (INT32U)NULL);
	drv_l1_scaler_output_format_set(C_USE_SCALER_NUMBER,img_decode_struct->output_format);
	...
	drv_l1_scaler_out_of_boundary_color_set(C_USE_SCALER_NUMBER,img_decode_struct->out_of_boundary_color);
