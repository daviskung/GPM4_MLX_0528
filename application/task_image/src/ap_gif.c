...
INT32S gif_input_data_read(void)
{
	...
	if ((!gif_read_index || gif_read_index>(C_GIF_DEC_FS_BUFFER_SIZE>>1)) && gif_write_index==0) {
		if (gif_source_type == TK_IMAGE_SOURCE_TYPE_FILE) {
			...
			result = osMessageGet(gif_fs_cmd_a.result_queue, 200);
	...
	if (gif_read_index && gif_read_index<(C_GIF_DEC_FS_BUFFER_SIZE>>1) && gif_write_index==(C_GIF_DEC_FS_BUFFER_SIZE>>1)) {
		if (gif_source_type == TK_IMAGE_SOURCE_TYPE_FILE) {
			...
			if ((error!=OS_NO_ERR) || (gif_write_index<0)) {
			#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
			result = osMessageGet(gif_fs_cmd_b.result_queue, 200);
			if((result.status != osEventMessage) || (result.value.signals <0)) {
			#endif
				DBG_PRINT("Failed to read data from gif file\r\n");
...
void gif_scaler_set_parameters(INT32U width, INT32U height, INT32U buff, INT8S *status_ptr, INT32U gif_scaler_mode)
{
	drv_l1_scaler_input_pixels_set(C_USE_SCALER_NUMBER,width, height);
	drv_l1_scaler_input_visible_pixels_set(C_USE_SCALER_NUMBER,width, height);

	if (gif_output_image_width && gif_output_image_height) {
	    drv_l1_scaler_output_pixels_set(C_USE_SCALER_NUMBER,(width<<16)/gif_output_image_width, (height<<16)/gif_output_image_height, gif_output_buffer_width, gif_output_buffer_height);
	} else {
    	...
    	if (gif_output_ratio == 0x0) {      // Fit to output buffer width and height
    		...
    		drv_l1_scaler_output_pixels_set(C_USE_SCALER_NUMBER,1<<16, 1<<16, gif_output_buffer_width, gif_output_buffer_height);
    		...
    		INT32U factor;
      		...
      		drv_l1_scaler_output_pixels_set(C_USE_SCALER_NUMBER,factor, factor, gif_output_buffer_width, gif_output_buffer_height);
      	}
    }

	//scaler_input_addr_set(buff, 0, 0);
	drv_l1_scaler_input_A_addr_set(C_USE_SCALER_NUMBER,buff, 0, 0);
	...
	if (gif_output_buffer_pointer) {
   	...
   	drv_l1_scaler_output_addr_set(C_USE_SCALER_NUMBER,gif_scaler_out_buffer, (INT32U)NULL, (INT32U)NULL);
   	drv_l1_scaler_fifo_line_set(C_USE_SCALER_NUMBER,gif_scaler_mode);
	...
	drv_l1_scaler_output_format_set(C_USE_SCALER_NUMBER,gif_output_format);
	drv_l1_scaler_out_of_boundary_mode_set(C_USE_SCALER_NUMBER,1);			// Use Use color defined in scaler_out_of_boundary_color_set()
	drv_l1_scaler_out_of_boundary_color_set(C_USE_SCALER_NUMBER,gif_out_of_boundary_color);
}
...
INT32S gif_decode_and_scale(IMAGE_DECODE_STRUCT *img_decode_struct)
{
	...
	INT16U pos_x, pos_y=0;
	INT8U  scaler_done;
	INT32U gif_buff_size, i;
	INT32U decode_line, scaler_status=0, scaler_mode;
