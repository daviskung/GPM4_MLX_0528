...
#define C_FS_Q_MAX				32//128
#define C_FS_MAX_MSG			32//128
...
static void FileSrvRead_NAND(P_TK_FILE_SERVICE_STRUCT para);
...
INT32U nand_ogg_read_index;
INT32U nand_temp_buf;
	...
	while(1)
	{
		...
		switch(msg_id)
		{
			...
			case MSG_FILESRV_FS_READ:
				...
				break;
			case MSG_FILESRV_NAND_READ:
				FileSrvRead_NAND((P_TK_FILE_SERVICE_STRUCT)para);
				break;
...
//				break;
			...
			case MSG_FILESRV_FLUSH:
                ...
                xQueueReset(((P_TK_FILE_SERVICE_STRUCT) para)->result_queue);
                status = C_FILE_SERVICE_FLUSH_DONE;
				xQueueSend(((P_TK_FILE_SERVICE_STRUCT) para)->result_queue, &status, 1);
            #endif
				break;
...
static void FileSrvMount(STMountPara *para)
{
	...
	if(para->result_queue)
	{
        xQueueSend(para->result_queue, &status, 1);     /* send return status */
	}
#endif
}
...
#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
    //DBG_PRINT("para->buf_size %x\r\n",para->buf_size);
    read_cnt = fs_read(para->fd, para->buf_addr, para->buf_size);
	if(para->result_queue)
	{
        xQueueSend(para->result_queue, &read_cnt, 1);     /* send return status */
	}
#endif
}
...
{
	INT32S read_cnt;
	INT32U sec_size = para->buf_size/512;
	INT32U offset = para->data_offset/512;
	INT32U shift = para->data_offset%512;
	nand_temp_buf = (INT32U)gp_malloc_align(para->buf_size+512,32);
	//DBG_PRINT("filenand %d %d %d\r\n",nand_ogg_read_index,sec_size, offset);
	read_cnt = NAND_APP_ReadSector(offset + nand_ogg_read_index, sec_size+1 ,nand_temp_buf);
	gp_memcpy((INT8S*)para->buf_addr,(INT8S*)(nand_temp_buf+shift),para->buf_size);

	if(read_cnt==0)
	{
		read_cnt = para->buf_size;
		nand_ogg_read_index += sec_size;
		//DBG_PRINT("@");
	}
	gp_free((void*)nand_temp_buf);
	if(para->result_queue)
	{
        xQueueSend(para->result_queue, &read_cnt, 1);     /* send return status */
	}
...
static void FileUserDefineRead(P_TK_FILE_SERVICE_STRUCT para)	// added by Bruce, 2008/10/27
{
	...
	if(para->result_queue)
	{
		...
		OSQPost(para->result_queue, (void *) read_cnt);
	#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        xQueueSend(para->result_queue, &read_cnt, 0);
    #endif
    }
}
...
static void FileSrvScanFileStart(STScanFilePara *para)
{
	...
	#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        xQueueSend(para->result_queue, &status, 0);
    #endif
	}
}
