...
static void image_block_write_task_entry(void const *p_arg);
...
static osMutexId image_enc_sw_sem;
#endif
...
#define BlockWriteStackSize                 		2048
...
static void image_enc_lock(void)
{
	...
	if(image_enc_sw_sem)
        ...
        #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        osMutexWait(image_enc_sw_sem, 0);
        #endif
}
...
static void image_enc_unlock(void)
{
	if(image_enc_sw_sem)
        ...
        #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        osMutexRelease(image_enc_sw_sem);
        #endif
}
...
static INT32U image_encode_block_read(IMAGE_ENCODE_ARGUMENT *encode_info, IMAGE_ENCODE_PTR *encode_ptr)
{
    ...
    gp_memset((INT8S *)&src, 0x00, sizeof(src));
	...
	para.boundary_color = image_scaler_info.OutBoundaryColor;
	scale_state = drv_l2_scaler_trigger(C_USE_SCALER_NUMBER, 1, (ScalerFormat_t *)&src, &para);
...
void image_encode_entrance(void)
{
   ...
   osThreadId id;
   osThreadDef_t img_enc_task={"image_enc_task", image_block_write_task_entry, osPriorityNormal, 1, BlockWriteStackSize};
   #endif
   if(task_create == 0){
		#if _OPERATING_SYSTEM == _OS_UCOS2
			...
			//Create a Encode Block Write task
			//ret = OSTaskCreate(image_block_write_task_entry, (void *) 0, &BlockWriteStack[BlockWriteStackSize - 1], TSK_ENCODE_BLOCK_WRITE);
            id = osThreadCreate(&img_enc_task, (void*) NULL);
            if(id != 0){
                task_create = 1;
            }
			if(image_enc_sw_sem == NULL)
			{
                image_enc_sw_sem = osMutexCreate(NULL);
...
void image_encode_exit(void)
{
	INT8U err;
		...
		encode_msg_queue_id = NULL;
		if(image_enc_sw_sem)
			vSemaphoreDelete(image_enc_sw_sem);
		image_enc_sw_sem = NULL;
	#endif
    task_create = 0;
}
...
INT32U image_encode_start(IMAGE_ENCODE_ARGUMENT arg)
{
	...
	if(process_status == 0){
		...
		if(input_mode == IMAGE_ENCODE_BLOCK_READ_WRITE)
		{
            ...
            image_encode_jpeg_422_header[0x260] = 0;
            image_encode_jpeg_422_header[0x261] = 0;
		}
		...
		if(input_mode == IMAGE_ENCODE_BLOCK_READ_WRITE)
		{
			if(encode_fd != -1)
			{
				...
				jpeg_status = fs_write(encode_fd, (INT32U)image_encode.OutputBufPtr, sizeof(image_encode_jpeg_422_header));
						...
						if(encode_fd != -1)
						{
							disk_size = vfsFreeSpace(use_disk);
	   						if(disk_size > (encode_size + sizeof(image_encode_jpeg_422_header))){
								...
								jpeg_status = fs_write(encode_fd, (INT32U)image_encode.OutputBufPtr, (encode_size + sizeof(image_encode_jpeg_422_header)));
...
static CODEC_START_STATUS image_encode_block_write(ENCODE_BLOCK_WRITE_ARGUMENT *encode_info)
{
	  ...
	  if (encode_fd >= 0) {
		   ...
		   if(disk_size > encode_size_ck){
		     fs_write(encode_fd, encode_info->encode_buffer_ptr, encode_size_ck);
		   }else{
		     write_state_error = 1;
