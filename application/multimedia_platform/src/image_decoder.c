...
extern osMessageQId image_task_fs_queue_a;
extern osMessageQId image_task_fs_queue_b;
#endif

extern TK_IMAGE_TYPE_ENUM image_file_judge_type(INT16S fd);

#if C_SCALER_L2_EN == 1
static ScalerFormat_t	jpeg_src;
static ScalerPara_t	jpeg_para;
#endif
#if (_OPERATING_SYSTEM == _OS_UCOS2)
static OS_EVENT *image_dec_sw_sem = NULL;
#elif (_OPERATING_SYSTEM == _OS_FREERTOS)
static osMutexId image_dec_sw_sem;
#endif
...
static void image_dec_lock(void)
{
	...
	if(image_dec_sw_sem)
        ...
        #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        osMutexWait(image_dec_sw_sem, 0);
        #endif
...
static void image_dec_unlock(void)
{
	if(image_dec_sw_sem)
        ...
        #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        osMutexRelease(image_dec_sw_sem);
        #endif
...
void image_decode_entrance(void)
{
	...
	if(image_dec_sw_sem == NULL)
	{
	  ...
	  #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        image_dec_sw_sem = osMutexCreate(NULL);
	  #endif
...
void image_decode_exit(void)
{
	...
	if(image_dec_sw_sem)
        ...
        #elif (_OPERATING_SYSTEM == _OS_FREERTOS)
        vSemaphoreDelete(image_dec_sw_sem);
        #endif
...
CODEC_START_STATUS image_decode_Info(IMAGE_INFO *info,MEDIA_SOURCE src)
{
    ...
    if(image_decode_info_ck == 0){
       ...
       if(image_decode_Info_state == 0){
         ...
         #if (_OPERATING_SYSTEM == _OS_UCOS2) || (_OPERATING_SYSTEM == _OS_FREERTOS)
    ...
    image_dec_lock();
	    ...
	    if(image_decode_ck == 0){
	       ...
	       if(image.source_type == SOURCE_TYPE_SDRAM){
		     ...
		     image.source_size = (INT32U)SOURCE.type_ID.temp;//(image_arg.OutputWidth*image_arg.OutputHeight)*2;     //jpg file size
	       }else{
...
void image_decode_stop(void)
{
	 ...
	 //msgQReceive(ApQ, &msg_id, (void *)NULL, sizeof(IMAGE_DECODE_STRUCT));
    #endif
     image_dec_unlock();
}
...
CODEC_START_STATUS image_scale_start(IMAGE_SCALE_ARGUMENT arg)
{
    ...
    if(image_scaler_ck == 0){
        ...
        if((image_scaler_arg.OutputBufWidth > 1024) || (image_scaler_arg.OutputWidth > 1024)){
				...
				src.input_y_addr = (INT32U)image_scaler_arg.InputBufPtr;
				src.input_u_addr = 0;
				src.input_v_addr = 0;
				...
				src.output_y_addr = (INT32U)image_scaler_arg.OutputBufPtr;
				src.output_u_addr = 0;
				src.output_v_addr = 0;
				...
				scaler_state = drv_l2_scaler_trigger(C_USE_SCALER_NUMBER, 1, (ScalerFormat_t *)&src, &para);
           #else
...
static INT32S jpeg_scaler_fifo_mode_flow_by_piece(INT16S fd, INT32U file_size, INT32U buff_addr)
{
	...
	while(1) {
		...
		{
			DBG_PRINT("JPEG execution timeout-1!\r\n");
			break;
		}
...
#if	C_SCALER_L2_EN == 1
	jpeg_src.input_format = C_SCALER_CTRL_IN_YUYV;
	jpeg_src.output_format = C_SCALER_CTRL_OUT_YUYV;
	jpeg_para.boundary_mode = 1;
	jpeg_para.boundary_color = 0x008080;
#else
...
#if	C_SCALER_L2_EN == 1
	jpeg_src.fifo_mode = temp;
	jpeg_src.input_y_addr = jpg_temp_out_buf;
	jpeg_src.input_u_addr = 0;
	jpeg_src.input_v_addr = 0;
	jpeg_src.input_width = img_extend_width;
	jpeg_src.input_height = img_extend_height;
	jpeg_src.input_visible_width = img_valid_width;
	jpeg_src.input_visible_height = img_valid_height;
	jpeg_src.scale_mode = C_SCALER_BY_RATIO;
	if (bRatio_4_3_to_16_9) {
		jpeg_src.output_width = 960;
	} else {
		jpeg_src.output_width = output_width;
	}
	jpeg_src.output_height = output_height-(pad_line_num<<1);
	jpeg_src.output_buf_width = output_width;
	jpeg_src.output_buf_height = output_height-pad_line_num;
	jpeg_src.output_y_addr = y_addr;
