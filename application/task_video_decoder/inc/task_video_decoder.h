...
typedef struct
{
    ...
    INT32U video_frame_rate;
    INT32U video_cur_time;
    INT32U video_cur_frame;
...
{\
	osEvent event;\
	INT32U mm = message;\
	event = osMessageGet(ack_mbox, 0);\
	err = osMessagePut(msg_queue, (INT32U)&mm, 0);\
	if(err != osOK)\
	{\
		DBG_PRINT("osMessagePut Fail!!!\r\n");\
		RETURN(STATUS_FAIL);\
	}\
	event = osMessageGet(ack_mbox, msc_time ? msc_time : osWaitForever);\
	if(event.status != osEventMessage)\
	{\
		DBG_PRINT("[%x] osMessageGet Ack Fail!!!\r\n", message);\
		RETURN(STATUS_FAIL);\
	}\
	if(event.value.v == ACK_FAIL)\
	{\
		DBG_PRINT("Return Fail!!!\r\n");\
		RETURN(STATUS_FAIL);\
	}\
}
 ...
 *                              E X T E R N A L                           *
 **************************************************************************/
extern osMessageQId vid_dec_state_q;
extern osMessageQId aud_dec_q;
extern osMessageQId vid_dec_q;
...
extern INT32S pscaler_start(INT32U in_width, INT32U in_height, INT32U buf_width, INT32U buf_height);
extern INT32S pscaler_stop();
...
extern INT32S vid_dec_get_current_number(void);
...
extern void video_decode_task_entry(void const *parm);
...
extern void audio_decode_task_entry(void const *param);

#define OSSchedLock()
#define OSSchedUnlock()
#endif
