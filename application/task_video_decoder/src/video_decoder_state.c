...
#define TSK_PRI_VID_PARSER      osPriorityHigh
#define TSK_PRI_VID_VID_STATE   osPriorityNormal
#define TSK_PRI_VID_VID_DEC     osPriorityAboveNormal
#define	TSK_PRI_VID_AUD_DEC     osPriorityNormal
...
typedef enum
{
    ...
    MSG_VIDEO_DECODE_GET_ONE_FRAME
...
/* global varaible */
osMessageQId vid_dec_state_q;
osMessageQId vid_dec_frame_q;
osMessageQId vid_dec_state_ack_m;
...
INT32S vid_dec_get_frame_rate(void)
{
	return p_vid_dec_para->video_frame_rate;
}
...
INT32S vid_dec_get_current_number(void)
{
	INT32S nRet;
	...
	nRet = (INT32S)MultiMediaParser_GetVidCurNumberOfVideo(p_vid_dec_para->media_handle);
	OSSchedUnlock();*/
	nRet = p_vid_dec_para->video_cur_frame;
Return:
	return nRet;
}
...
INT32S vid_dec_get_one_frame(avi_frame_info *video_frame)
{
	INT8U  err;
	INT32S nRet, msg;
	INT32U  msg_id;
	...
	if(vid_dec_get_status() & C_VIDEO_DECODE_PARSER)
	{
		...
		msg_id = MSG_VIDEO_DECODE_GET_ONE_FRAME;
		err = osMessagePut(vid_dec_state_q, (INT32U)&msg_id, 0);
		if(err != osOK)
		{
			DEBUG_MSG("osMessagePut Fail!!!\r\n");
			RETURN(STATUS_FAIL);
		}

		event = osMessageGet(vid_dec_frame_q, osWaitForever);
        if (event.status != osEventMessage)
        {
        	DEBUG_MSG("osMessageGet Fail!!!\r\n");
			RETURN(STATUS_FAIL);
        }
        avi_frame = (avi_frame_info *)event.value.v;
        video_frame->AddrFrame = avi_frame->AddrFrame ;
        video_frame->FrameSize = avi_frame->FrameSize ;
        video_frame->Duration = avi_frame->Duration ;
		//nRet = msg_id;
	}
Return:
...
INT32S video_decode_state_task_create(INT8U pori)
{
	...
	INT32S nRet;
	osThreadId id;
	osMessageQDef_t vid_dec_state_qdef = {C_VID_DEC_QUEUE_MAX_LEN, sizeof(void*),NULL};
	osMessageQDef_t vid_dec_frame_q_buffer = {C_VID_DEC_QUEUE_MAX_LEN, sizeof(void*),NULL};
	osMessageQDef_t vid_dec_state_ack_qdef = {1, sizeof(void*),NULL};
	osThreadDef_t vid_dec_thread_def = { "video decode state", video_decode_state_task_entry, pori, 0, C_VID_DEC_STATE_STACK_SIZE*4 };

	p_vid_dec_para = &vid_dec_para;
	gp_memset((INT8S*)p_vid_dec_para, 0, sizeof(video_decode_para_struct));
    vid_dec_state_q = osMessageCreate(&vid_dec_state_qdef,NULL);
    if(!vid_dec_state_q) {
    	RETURN(STATUS_FAIL);
    }

    vid_dec_frame_q = osMessageCreate(&vid_dec_frame_q_buffer,NULL);
    if(!vid_dec_frame_q) {
    	RETURN(STATUS_FAIL);
    }


    vid_dec_state_ack_m = osMessageCreate(&vid_dec_state_ack_qdef,NULL);
	if(!vid_dec_state_ack_m) {
		RETURN(STATUS_FAIL);
	}

	id = osThreadCreate(&vid_dec_thread_def, NULL);
	if(id == 0) {
		RETURN(STATUS_FAIL);
	}
...
INT32S video_decode_state_task_del(INT8U pori)
{
	...
	nRet = STATUS_OK;
	//error handling will stop decoder task
	if (vid_dec_get_status() & C_VIDEO_DECODE_ERR) {
		while(vid_dec_get_status() & C_VIDEO_DECODE_ERR) //wait error handling
			osDelay(1);
...
Return:
    ...
    vQueueDelete(vid_dec_state_q);

    OSQFlush(vid_dec_frame_q);
    vQueueDelete(vid_dec_frame_q);

    vQueueDelete(vid_dec_state_ack_m);
...
INT32S vid_dec_end_callback(INT32U bflag)
{
	...
	if(bflag == C_VIDEO_DECODE_PLAYING) {
		DEBUG_MSG("%s>>VIDEO.\r\n", __func__);
		vid_dec_stop_timer();
		msg = MSG_VIDEO_DECODE_VID_END;
		err = osMessagePut(vid_dec_state_q, (INT32U)&msg, 0);
	}

	if(bflag == C_AUDIO_DECODE_PLAYING) {
		DEBUG_MSG("%s>>AUDIO.\r\n", __func__);
		msg = MSG_VIDEO_DECODE_AUD_END;
		err = osMessagePut(vid_dec_state_q, (INT32U)&msg, 0);
	}
...
static INT32S viddec_read(INT16S fd, INT32U fs_buf, INT32U cblen)
{
	...
	if(copyflag) {
		...
		ret = fs_read(fd, temp_buf, temp_len);
		if(ret < 0) {
			goto __exit;
		}
...
__exit:
	...
	return ret;
#else
	INT32S ret = fs_read(fd, fs_buf, cblen);
...
static int viddec_store(INT32U work_buffer, INT32U len)
{
	...
	fid = fs_open("C:\\VidPlayingState.bin", O_CREAT|O_TRUNC|O_WRONLY);
	if(fid < 0) {
		while(1);
	}

	ret = fs_write(fid, work_buffer, len);
	fs_close(fid);
...
static int viddec_load(INT32U work_buffer, INT32U len)
{
	...
	fid = fs_open("C:\\VidPlayingState.bin", O_RDONLY);
	if(fid < 0) {
		while(1);
	}

	ret = fs_read(fid, work_buffer, len);
	fs_close(fid);
...
void video_decode_state_task_entry(void const *para)
{
	void    *hMedia=NULL;
	INT8U   err;
    ...
    {
    	osEvent event;
    	event = osMessageGet(vid_dec_state_q, osWaitForever);
    	if (event.status != osEventMessage) {
        	continue;
        }
        ...
        case MSG_VIDEO_DECODE_PARSER_HEADER_START:
			...
			if(p_vid_dec_para->bResumePlayFlag & C_RESUME_PLAY) {
           	...
           	if((error_id & 0x80000000) || !hMedia) {
           		...
           		fs_close(p_vid_dec_para->file_handle);
	           	p_vid_dec_para->file_handle = -1;
	           	p_wave_info = NULL;
           		p_bitmap_info = NULL;
           		msg_id = ACK_FAIL;
                osMessagePut(vid_dec_state_ack_m, (INT32U)&msg_id, 0);
           	} else {
			 	...
			 	p_vid_dec_para->video_frame_rate = MultiMediaParser_GetVidFrameRate(p_vid_dec_para->media_handle);
				p_vid_dec_para->video_cur_time = MultiMediaParser_GetVidCurTime(p_vid_dec_para->media_handle) >> 8;
        ...
        case MSG_VIDEO_DECODE_SET_PLAY_SPEED:
        	...
        	if(set_play_speed() < 0) {
				//if reach audio stream end, continue as success.
        ...
        case MSG_VIDEO_DECODE_ERROR:
        	...
        	vid_dec_parser_end();
           	...
           	OSQFlush(vid_dec_state_q);
           	vid_dec_clear_status(C_VIDEO_DECODE_ALL); //important, clear flag after flush queue
        	break;
        ...
        case MSG_VIDEO_DECODE_EXIT:
        	...
        	break;
        case MSG_VIDEO_DECODE_GET_ONE_FRAME:
        	// get video raw data
			...
			//while(error_id<0);
			num = MultiMediaParser_GetNumberOfFrameInVidBuf(p_vid_dec_para->media_handle);
			avi_frame.FrameSize = size;
			avi_frame.AddrFrame = raw_data_addr;
			avi_frame.Duration = Duration;
			p_avi_frame = (INT32U)&avi_frame;
			if(!raw_data_addr || size <= 0)
			{
				if(MultiMediaParser_IsEOV(p_vid_dec_para->media_handle))
				{
					avi_frame.FrameSize = 0xFFFFFFFF;         //表示decode已经结束了 
                    osMessagePut(vid_dec_frame_q, (INT32U)(&p_avi_frame), 0);
				}
				else
				{
					avi_frame.FrameSize = 0xFFFFFFFE;         //
					osMessagePut(vid_dec_frame_q, (INT32U)(&p_avi_frame), 0);
				}
			}
			else
...
INT32S qtff_decode_thumbnail(CHAR *filename, INT32U Fmt, INT16U width, INT16U height, INT32U decode_addr)
{
	...
	// get thumb atom
	while(length > 0) {
		ret = fs_read(fd, (INT32U)&header, sizeof(ATOM_HDR_t));
		if(ret <= 0) {
			ret = STATUS_FAIL;
			goto __exit;
		}
...
__exit:
	if(dec_flag) {
		...
		ret = fs_read(fd, (INT32U)raw_data_addr, sub_len);
		if(ret <= 0) {
			ret = STATUS_FAIL;
			goto __exit;
		}
