...
#define romNVIC                 ( *( ( volatile unsigned long * ) 0xE00FF000 ) )
#define romDWT                  ( *( ( volatile unsigned long * ) 0xE00FF004 ) )
#define romFPB                  ( *( ( volatile unsigned long * ) 0xE00FF008 ) )
#define romITM                  ( *( ( volatile unsigned long * ) 0xE00FF00C ) )
#define romTPIU                 ( *( ( volatile unsigned long * ) 0xE00FF010 ) )
#define romETM                  ( *( ( volatile unsigned long * ) 0xE00FF014 ) )
#define romEND                  ( *( ( volatile unsigned long * ) 0xE00FF014 ) )
...
void vConfigureTimerForRunTimeStats( void )
{
    ...
    /* Enable TRCENA. */
	rtsSCB_DEMCR |= rtsTRCENA_BIT;

	/* Reset counter. */
	rtsDWT_CYCCNT = 0;

	/* Enable counter. */
	rtsDWT_CONTROL |= rtsCOUNTER_ENABLE_BIT;

	//printf("rtsSCB_DEMCR=0x%08x rtsDWT_CONTROL=0x%08x\r\n", rtsSCB_DEMCR, rtsDWT_CONTROL);
	//printf("romNVIC=0x%08x romDWT=0x%08x romITM=0x%08x romETM=0x%08x romEND=0x%08x\r\n", romNVIC, romDWT, romITM, romETM, romEND);
}
/*-----------------------------------------------------------*/
