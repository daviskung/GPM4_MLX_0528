...
typedef struct{  INT16U Scalar, ToUpper; } UCD_TOUPPER_TABLE;

UCD_TOUPPER_TABLE const UCD_toupper[] =
{
  { 0x00B5, 0x039C },
  { 0x00E0, 0x00C0 },
  { 0x00E1, 0x00C1 },
...
UTF32 unicode_toupper(UTF32 Ch)
{
  ...
  //return Ch;	//pass a bug about search in the UCD_toupper table, m by zy 05-01-12 
  //wanghuidi temp deleted,2005.01.18

  /* Binary search in the UCD_toupper table */
  if ((Ch >= 0x00B5) && (Ch <= 0x24E9))
  {
    INT16U Semi, Lo = 0, Hi = 623-1;//(sizeof(UCD_toupper) >> 2) - 1;		//060511 zhangzha modified 

    for (;;)
    {
		 if (UCD_toupper[Lo].Scalar > Ch) break;
		 if (UCD_toupper[Lo].Scalar == Ch) return UCD_toupper[Lo].ToUpper;
		 if (UCD_toupper[Hi].Scalar < Ch) break;
		 if (UCD_toupper[Hi].Scalar == Ch) return UCD_toupper[Hi].ToUpper;

		 Semi = (Lo + Hi) >> 1;
		 if (UCD_toupper[Semi].Scalar == Ch) return UCD_toupper[Semi].ToUpper;
		 if (Hi - Lo == 0) break;
		 if (UCD_toupper[Semi].Scalar < Ch) Lo = Semi+1;//Lo = Semi;	//060511 zhangzha modified 
			 else Hi = Semi-1;//Hi = Semi;								//060511 zhangzha modified 
    }
  }
