...
#define FS_SMALL_CAPACITY_MEMORY    1       //支持RAM小的IC，功能沒有影響，但是在某些情況下，性能可能會下降。 
#else
#define FS_SMALL_CAPACITY_MEMORY    0       //支持RAM小的IC，功能沒有影響，但是在某些情況下，性能可能會下降。 
#endif
...
#define _READ_WRITE_SPEED_UP		1		//表示读或写一个sector时不需通过buffer直接对disk进行读写
    ...
    #define _SEEK_SPEED_UP				0		//此定义表示是否对seek加速，需要额外的内存空间和堆栈
    #define _CACHE_FAT_INFLEXION		0		//基于拐点的seek加速方法
#else
...
//============================================================================
  ...
  #define _FS_DEFINE_
      ...
      #define FS_NOR      7
      #if FS_SMALL_CAPACITY_MEMORY == 1
        #define FS_DEV_MAX  3         /*在RAM小的时候，disk不需要同时支持那么多，故这里减小
                                        改小后，请注意修改FileSysDrv[]*/
      #else
        #define FS_DEV_MAX  8
      #endif
/* DEV MODE */
      ...
      #define USB_NUM           0                 //支持的usb host的设备数, Maximum number is 4
...
#if WITHEXFAT == 1
#define _READ_WRITE_SPEED_UP 1				//ExFat mast used
#endif
