 ...
 **************************************************************************/
/* cdsp input source */
#define C_CDSP_DVP		    0x0 //Parallel sensor
#define C_CDSP_SDRAM		0x1
#define C_CDSP_MIPI			0x2
#define C_CDSP_MIPI1		0x3
#define C_CDSP_SKIP_WR		0x4
...
typedef struct gpCdspAE_s
{
	...
	INT16U  ae_meter;
	//float   ae_user_weight_table[64];//user define ae weight table
	float   *ae_user_weight_table;//user define ae weight table
} gpCdspAE_t;
...
{
	unsigned short max_rgain;
	unsigned short max_bgain;
	unsigned short min_rgain;
	unsigned short min_bgain;
}gpCdspWBGain_t;

typedef struct gpCdspCV_denoise_s
{
	unsigned char raw_enable;
	unsigned char w1;
	unsigned char w2;
	unsigned char w3;
	unsigned char yuv_enable;
	unsigned char y_mode;
	unsigned char uv_mode;
	unsigned char HAvg_Y_mode;
	unsigned char HAvg_U_mode;
	unsigned char HAvg_V_mode;
}gpCdspCV_denoise_t;

typedef struct gpCdspCV_denoise_control_s
{
    unsigned char        is_open_cv_denoise;
    gpCdspCV_denoise_t  cv_denoise_para[5];
}gpCdspCV_denoise_control_t;

typedef struct gpCdspCV_ev_control_s
{
    unsigned char is_ev_control;
    unsigned char ev_control_threshold;
}gpCdspCV_ev_control_t;


typedef struct gpCdspCV_Para_s
{
    gpCdspCV_denoise_control_t  cv_denoise_para;
    gpCdspCV_ev_control_t       cv_ev_control;
}gpCdspCV_Para_t;
...
typedef struct gpCdspDev_s
{
	...
	/* AE buffer */
	volatile unsigned int aeAddr[2][64];
	...
	//ae, awb
	INT16U ae_win_w, ae_win_h;

	INT16U ae_fr_thr, ae_fr_cnt;
	INT16U awb_fr_thr, awb_fr_cnt;
	INT16U rgain, bgain, global_gain;;

	INT16S high_lum_switch_cnt;
    INT16S low_lum_switch_cnt;
	...
	void (*sensor_set_exptime)(int);

    //INT8U   *lpCV_DeNoise_para;
    gpCdspCV_Para_t cv_para;
	gpCisCali_t sensor_cdsp;
...
extern void drv_l2_CdspTableRegister(gpCisCali_t *sensor);
...
extern void drv_l2_set_raw_capure_setting(INT32U lincor_lencmp, INT32U cbit);
extern void drv_l2_set_target_sensor_size(INT16U hsize, INT16U vsize);
extern INT32S drv_l2_cdsp_set_linear_correction(void);
extern INT32U drv_l2_cdsp_set_lens_compensation(void);
extern INT32S drv_l2_cdsp_ae_is_night(void);
