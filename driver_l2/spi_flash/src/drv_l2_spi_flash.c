...
#define C_SPI_CS_PIN		IO_D6//IO_A0			// IOF1
 ...
 *                              M A C R O S                               *
 **************************************************************************/
#define CLEAR(ptr, cblen)		memset((INT8S *)ptr, 0x00, cblen)
...
{
#if 1
	osSemaphoreWait(spi_sem, osWaitForever);
#endif
}
...
#if 1
	if(spi_sem == 0) {
		spi_sem = osSemaphoreCreate(osSemaphore(spi_sem), 1);
		if(spi_sem == 0) {
			return STATUS_FAIL;
		}
	}
