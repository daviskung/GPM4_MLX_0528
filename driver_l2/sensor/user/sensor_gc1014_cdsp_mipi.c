...
#define GC1014_30FPS_50HZ_EXP_TIME_TOTAL		209
...
static mipi_config_t gc1014_mipi_cfg =
{
	...
	4,
	#else
	8,//2, 					/* back porch, 0~0xF */ //If Set 4 have show paper color at left side.u2.20160119
	0,
	#endif
...
//30fps
	...
	{0xF8, 0x03}, //{0xF8, 0x03}, // Pll mode 2
//~
...
static int gc1014_cvt_agc_gain(int agc_gain)
{
    ...
    if(gc1014_analog_gain>7200)   gc1014_analog_gain=7200; // max_gain=7.2
    //if(gc1014_analog_gain>5200)   gc1014_analog_gain=5200; // max_gain=5.2
	...
	if(gc1014_analog_gain>4000)
	{
      ret = gc1014_sccb_write(0x66,  0x1c);
      if(ret < 0) return ret;
	}
	if(gc1014_analog_gain<3250)
    {
      ret = gc1014_sccb_write(0x66,  0x20);
      if(ret < 0) return ret;
	}
...
#ifdef MIPI_ISR_TEST
static void mipi_gc1014_handle(INT32U event)
{
...
static void gc1014_mipi_sensor_init(void)
{
    ...
    //GC1014_set_favorite();
	// mclk output
	//drv_l2_sensor_set_mclkout(gc1014_cdsp_mipi_ops.info[0].mclk);
...
void gc1014_cdsp_mipi_init(void)
{
    ...
    DBG_PRINT("Sensor GC1014 cdsp mipi init completed\r\n");
}
...
/*	//Only 720p
	...
	{
	case 0:
        gc1014_sccb_write_table((regval16_t *)gc1014_scale_vga_table);
		break;

	case 1:
		gc1014_sccb_write_table((regval16_t *)gc1014_720p_table);
		break;
...
*/
...
#if 1   // change mclk
	drv_l2_sensor_set_mclkout(gc1014_cdsp_mipi_ops.info[index].mclk);
#else
	...
	case 1:
		...
		gc1014_mipi_cfg.data_type = MIPI_RAW8;
		//gc1014_mipi_cfg.h_back_porch = 4;
		break;
	...
	case 3:
		...
		//gc1014_mipi_cfg.h_back_porch = 2;//4;
        gc1014_mipi_cfg.pixel_clk_sel = 0;
		break;
 ...
 * @return 	none
 */
static void gc1014_cdsp_mipi_stream_off(void)
{
