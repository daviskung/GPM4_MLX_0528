...
#define TFT_CLOCK_SOURCE            13
#define PPU_CLOCK_SOURCE            19
#define FRAME_BUFFER_CLOCK_SOURCE   22
#define DISPLAY_REG_CLOCK_SOURCE    27
#define TFT_CLOCK_ENABLE            1
#define TFT_CLOCK_DISABLE           0

static INT16S drv_l1_tft_clock_set(INT16U enable)
{
    INT16S ret,value = 0;

    ret = drv_l1_clock_set_system_clk_en(DISPLAY_REG_CLOCK_SOURCE, enable);
    if(ret)
        value = ret;
    ret = drv_l1_clock_set_system_clk_en(FRAME_BUFFER_CLOCK_SOURCE, enable);
    if(ret)
        value = ret;
    ret = drv_l1_clock_set_system_clk_en(PPU_CLOCK_SOURCE, enable);
    if(ret)
        value = ret;
    ret = drv_l1_clock_set_system_clk_en(TFT_CLOCK_SOURCE, enable);
    if(ret)
        value = ret;
...
static void drv_l1_tft_msec_wait(INT32U msec)
{
#if 1
...
{
    drv_l1_tft_clock_set(TFT_CLOCK_ENABLE);
    R_TFT_CTRL = 0;
    drv_l1_tft_slide_en_set(FALSE);
}

/**
 * @brief   Uninitialize TFT controller
 * @param   none
 * @return 	none
 */
void drv_l1_tft_uninit(void)
{
    R_TFT_CTRL = 0;
    drv_l1_tft_clock_set(TFT_CLOCK_DISABLE);
}
...
/**
 * @brief   set TFT target clock, read data
 * @param   tar_clk[in]: target clock, unit:Hz
 * @return 	none
 */
INT16U drv_l1_tft_target_clk_set(INT32U tar_clk)
{
    INT32U clock_div;

    clock_div = MCLK / tar_clk;
    clock_div = ((clock_div-1)<<1);
    if(clock_div > TFT_CLK_DIVIDE_32)
        clock_div = TFT_CLK_DIVIDE_32;
    drv_l1_tft_clk_set(clock_div);
}
