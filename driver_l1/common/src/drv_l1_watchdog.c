...
void watchdog_init(void)
{
    ...
    R_SYSTEM_WATCHDOG_CLEAR = 0xA005;  /* clear watchdog flag */
    //R_SYSTEM_RESET_FLAG = 0x0010;  /* reset watchdog timeout flag */
    R_SYSTEM_RESET_FLAG = 0x0018;  /* reset watchdog timeout flag */
}
...
INT32U lvr_ctrl(INT32U LVROFF)
{
    ...
    R_SYSTEM_LVR_CTRL &= ~0xF0;
    R_SYSTEM_LVR_CTRL |= (LVROFF<<4);    //0xD0000020
    return WATCHDOG_OK;
}
#endif

//INT32U watchdog_ctrl(INT32U WDGPD, INT32U WDGS, INT32U WDGEN)
void watchdog_ctrl(INT32U WDGPD, INT32U WDGS, INT32U WDGEN)
{
    //INT32U trace;
    //trace = (WDGPD | WDGS<<14 | WDGEN<<15);
    //DRV_Reg32(WATCHDOG_CTRL_ADDR) = (WDGPD | WDGS<<14 | WDGEN<<15);
    R_SYSTEM_WATCHDOG_CTRL = (WDGPD | WDGS<<14 | WDGEN<<15);
    //return WATCHDOG_OK;
}
...
INT32U watchdog_clear(void)
{
   ...
   if ( (R_SYSTEM_WATCHDOG_CTRL & WATCHDOG_ENABLE_MASK) == WATCHDOG_ENABLE_MASK) /*When watch dog enabled*/
   {
    //DRV_Reg32(WATCHDOG_CLEAR_ADDR) = (WATCHDOG_CLEAR_VALUE & 0x0000FFFF);
    R_SYSTEM_WATCHDOG_CLEAR = (WATCHDOG_CLEAR_VALUE & 0x0000FFFF);
    return WATCHDOG_OK;
    }
