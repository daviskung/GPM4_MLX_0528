...
BOOLEAN gpio_init_io(INT32U port, BOOLEAN direction)
{
    volatile INT16U gpio_set;
    volatile INT16U gpio_set_num;
    //INT32U trace;
...
{
	R_SYSTEM_HDMI_CTRL |= 0x8000;	//  HDMI EN
	R_SYSTEM_CLK_EN0  |= 0x8;			// HDMI CLK source
	R_HDMITXPHYCONFIG1 &= ~0x1;          //HDMI power down
	 R_TFT2_CTRL &=(~0x10000);
	R_SYSTEM_HDMI_CTRL &= (~0x8000);	//  HDMI disable
	R_SYSTEM_CLK_EN0  &= (~0x8);			// CLK source
