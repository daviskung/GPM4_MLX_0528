...
void ppu_init(void)
{
	...
	R_PPU_IRQ_EN |= (C_PPU_INT_PEND_PPU_VBLANK | C_PPU_INT_PEND_DMA_COMPLETE);
...
INT32S update_ppu_registers(void)
{
	...
	if (flag & C_UPDATE_REG_SET_PPU) {
		...
		if(R_PPU_MISC & 0x2)
		{
            value = new_register_sets_ptr->ppu_misc;
            value |= 0x2;
		}
#endif
		...
		R_PPU_DEFLICKER_PARA = new_register_sets_ptr->ppu_deflicker_para;
	}
...
#elif 0//_OPERATING_SYSTEM == _OS_FREERTOS
    if (mask == 0) {						// Clear device interrupt mask
		NVIC_EnableIRQ(PPU_IRQn);
	} else if (mask == 1) {
		NVIC_DisableIRQ(PPU_IRQn);
