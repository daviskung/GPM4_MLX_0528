 ...
 * @return	none
 */
void drv_l1_FrontYuvOrderSet(INT32U mode)
{
...
{
	if(hPol) {
		R_CDSP_FRONT_CTRL1 |= (1 << 8);
	} else {
		R_CDSP_FRONT_CTRL1 &= ~(1 << 8);
	}

	if(vPol) {
		R_CDSP_FRONT_CTRL1 |= (1 << 9);
	} else {
		R_CDSP_FRONT_CTRL1 &= ~(1 << 9);
	}
 ...
 * @return	none
 */
void drv_l1_FrontInputGate(INT32U tggz)
{
...
void drv_l1_FrontClrIntEvent(INT32U clrintevt)
{
	...
	R_CDSP_FRONT_VDRF_INT = (INT8U)((clrintevt >> 8) & 0x03);
}
...
void drv_l1_FrontSetMIPIFormat(unsigned char input_format)
{
	/* only bit[0]= 0 = default = RAW mode, else YUV = 1*/
	/*By RD Ivan suggestion*/

	R_CDSP_MIPI_CTRL &= 0xfff0fffC;

	if(input_format == 1)  /*Raw*/
	{
        R_CDSP_MIPI_CTRL |= 0;
    }
    else	/*yuv*/
...
void drv_l1_FrontHdCntSet(INT32U value)
{
