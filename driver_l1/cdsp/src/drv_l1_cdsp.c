 ...
 * @param	path[in]: data input source, FRONT_INPUT, SDRAM_INPUT, MIPI_INPUT(MIPI0), MIPI1_INPUT(MIPI1)
...
/*
    //Select MCLK port:IOC9
    R_FUNPOS1 &= ~0x03F00000;
    R_FUNPOS1 |= (MCLK_IOC9| 1<< MCLK_IOC9_2_CDSP);
*/
	// disable
	if(enable == 0) {
...
/**
 * @brief	Front set interrupt enable
 * @param	inten[in]: enable or disable
 * @param	bit[in]: VD FALL, VD RISE
 * @return	none
 */
void drv_l1_FrontSetIntEn(INT8U inten,INT8U bit)
{
	if (inten)
	{
		//R_CDSP_FRONT_VDRF_INT |= R_CDSP_FRONT_VDRF_INT;	//clear
        R_CDSP_FRONT_VDRF_INT |= bit;	//clear
		R_CDSP_FRONT_VDRF_INTEN |= bit;	//enable
	}
	else
	{
		R_CDSP_FRONT_VDRF_INTEN &= ~bit;	//disable
		R_CDSP_FRONT_VDRF_INT |= bit;   	//clear
        //R_CDSP_FRONT_VDRF_INT |= R_CDSP_FRONT_VDRF_INT;	//clear
	}
}
...
/**
* @brief	front interrupt clear status
* @param	bit [in]: clear interrupt bit
* @return	none
*/
void drv_l1_FrontClrIntStatus(INT8U bit)
{
	R_CDSP_FRONT_VDRF_INT = bit;
}
...
{
	INT32U val=0;
	// 0:front, 1:sdram, 2:mipi(mipi0&mipi1)
	val = R_CDSP_DO;
	val &= ~(0x3<<4);

	if ((image_source == 2)||(image_source == 3)){
	    image_source = 2;
	}
    val |= (image_source << 4);
	R_CDSP_DO = val;
 ...
 * @param	yuv_format[in]: set out sequence, 0:V0Y1U0Y0(pscale:VYUV), 1:Y1V0Y0U0(YVYU), 2:U0Y1V0Y0(UYVY), 3:Y1U0Y0V0(YUYV)
 * @return	none
 */
...
void drv_l1_CdspSetReadBackSize10(INT16U hoffset, INT16U voffset, INT16U hsize, INT16U vsize)
{
	R_CDSP_WDRAM_HOFFSET = 0;//hoffset;
	R_CDSP_WDRAM_VOFFSET = 0;//voffset;
 ...
 * @return	none
 */
void drv_l1_CdspSetLiCor(INT8U *plicordata)
{
	INT32U i;
 ...
 * @return	none
 */
void drv_l1_CdspSetHr0(INT32U value)
{
	INT32S i;
 ...
 * @return	none
 */
void drv_l1_CdspSetHr1(INT32U value)
{
	INT32S i;
 ...
 * @return	none
 */
void drv_l1_CdspSetHr2(INT32U value)
{
	INT32S i;
 ...
 * @return	none
 */
void drv_l1_CdspSetHr3(INT32U value)
{
	INT32S i;
 ...
 * @return	none
 */
void drv_l1_CdspSetLucMaxTan8SlopCLP(INT16U *pmaxtan8data, INT16U *pslopdata, INT16U *pclpdata)
{
	INT32U i;
 ...
 * @return	none
 */
void drv_l1_CdspSetRadusF0(INT16U *pradusf0data)
{
	INT32U i;
 ...
 * @return	none
 */
void drv_l1_CdspSetRadusF1(INT16U *pradusf1data)
{
	INT32U i;
 ...
 * @return	none
 */
void drv_l1_CdspSetLenCmp(INT32U lcen,INT32U stepfactor)
{
...
void drv_l1_CdspSetWb_RB_Gain2(INT16U rgain2, INT16U bgain2)
{
	R_CDSP_AWB_WIN_RGGAIN2 &= (~0x1FF);
	R_CDSP_AWB_WIN_RGGAIN2 |= (rgain2 & 0x1FF);
	R_CDSP_AWB_WIN_BGAIN2 &= (~0x1FF);
	R_CDSP_AWB_WIN_BGAIN2 |= bgain2 & 0x1FF;
}
...
{
	R_CDSP_LH_DIV_CTRL &= ~0x1ffff;
	R_CDSP_LH_DIV_CTRL = (lhdiv & 0x07) | ((lhtdiv& 0x07) << 4) |
						(((INT32U)lhcoring & 0xFF) << 8) |
...
void drv_l1_CdspGetEdgeLCoring(INT8U *lhdiv, INT8U *lhtdiv, INT8U *lhcoring, INT8U *lhmode)
{
	...
	*lhcoring = (R_CDSP_LH_DIV_CTRL >> 8) & 0xFF;
...
{
	R_CDSP_YUVSP_EFFECT_OFFSET = (y_offset &0xFF) | (((INT32U)u_offset & 0xFF) << 8) |
...
{
	R_CDSP_YUVSP_EFFECT_SCALER = (y_scale & 0xFF) | (((INT32U)u_scale & 0xFF) << 8) |
...
{
	*y_scale = R_CDSP_YUVSP_EFFECT_SCALER & 0xFF;
...
{
	R_CDSP_HUE_ROT_U = ((u_huecosdata & 0xFF) << 8) | (u_huesindata & 0xFF);
	R_CDSP_HUE_ROT_V = ((v_huecosdata & 0xFF) << 8) | (v_huesindata & 0xFF);
 ...
 * @return	none
 */
void drv_l1_CdspSetImageSize(INT16U hsize, INT16U vsize)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetLinCorrEn(INT8U lincorr_en)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetDbpcSel(INT8U algorithm_sel)
{
	if (algorithm_sel == 0){
 ...
 * @return	none
 */
void drv_l1_CdspSetDpcEn(INT8U dpc_en,INT8U algorithm_sel)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetDpcRcvModeSelThr(INT8U sel_mode, INT16U DPCth1, INT16U DPCth2, INT16U DPCth3)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetSmoothFactor(INT16U DPCn)
{
	R_ISP_LI_HR_CTRL &= ~0x1FF00;
 ...
 * @return	none
 */
void drv_l1_CdspSetCrostlkGbGrEn(INT8U ctk_en, INT8U ctk_gbgr)
{
	if (ctk_en == 1) {
 ...
 * @return	none
 */
void drv_l1_CdspSetCrostlkThold(INT16U ctk_thr1, INT16U ctk_thr2, INT16U ctk_thr3, INT16U ctk_thr4)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetCrostlkWeight(INT16U ctkw1, INT16U ctkw2, INT16U ctkw3)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetDenoiseEn(INT8U denoise_en)
{
	if (denoise_en == 1) {
 ...
 * @return	none
 */
void drv_l1_CdspSetDenoiseThold(INT16U rdn_thr1,INT16U rdn_thr2,INT16U rdn_thr3,INT16U rdn_thr4)
{
 ...
 * @return	none
 */
void drv_l1_CdspSetDenoiseWeight(INT16U rdnw1, INT16U rdnw2, INT16U rdnw3)
{
...
{
	R_CDSP_AWB_SPECWIN_Y_THR = Ythr0|(INT32U)(Ythr1 << 8)|(INT32U)(Ythr2 << 16)|(INT32U)(Ythr3 << 24);
}
...
)
{
	...
	p_tb_val = table_val;
	p_sram_hw = &R_CDSP_MACRO_RW;

    reg_tmp = R_CDSP_MACRO_CTRL;
    R_CDSP_MACRO_CTRL = 0x1F1F;
    ...
    do {
        ...
        t1 = *p_sram_hw++;   t2 = *p_sram_hw++;   t3 = *p_sram_hw++;   t4 = *p_sram_hw++;
        *p_tb_val++ = t1;   *p_tb_val++ = t2;   *p_tb_val++ = t3;   *p_tb_val++ = t4;
...
INT32S drv_l1_CdspGetAwbSumCnt(INT8U section, INT32U *sumcnt)
{
	...
	if(section == 1) {
		*sumcnt = R_CDSP_SUM_CNT1;
		//*sumcnt = ((INT32U)(pCdspReg2->cdspAwbSumCnt1[3]&0x03)<<24)|((INT32U)pCdspReg2->cdspAwbSumCnt1[2]<<16)|((INT32U)pCdspReg2->cdspAwbSumCnt1[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumCnt1[0];
	} else if(section == 2) {
		//*sumcnt = ((INT32U)(pCdspReg2->cdspAwbSumCnt2[3]&0x03)<<24)|((INT32U)pCdspReg2->cdspAwbSumCnt2[2]<<16)|((INT32U)pCdspReg2->cdspAwbSumCnt2[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumCnt2[0];
		*sumcnt = R_CDSP_SUM_CNT2;
	} else if(section == 3) {
		//*sumcnt = ((INT32U)(pCdspReg2->cdspAwbSumCnt3[3]&0x03)<<24)|((INT32U)pCdspReg2->cdspAwbSumCnt3[2]<<16)|((INT32U)pCdspReg2->cdspAwbSumCnt3[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumCnt3[0];
...
INT32S drv_l1_CdspGetAwbSumRG(INT8U section, INT32U *sumrgl, INT32S *sumrgh)
{
	...
	if(section == 1) {
		//*sumrgl = ((INT32U)pCdspReg2->cdspAwbSumRg1L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumRg1L[2] << 16)
		//	| ((INT32U)pCdspReg2->cdspAwbSumRg1L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumRg1L[0];
		//*sumrgh = (SINT32)(pCdspReg2->cdspAwbSumRg1H&0x07);//<<16)|((INT32U)pCdspReg2->cdspAwbSumRg1L[3]<<8)|((INT32U)pCdspReg2->cdspAwbSumRg1L[2]);
		*sumrgl = R_CDSP_SUM_RG1_L;
		*sumrgh = R_CDSP_SUM_RG1_H;
	} else if(section == 2) {
		//*sumrgl = ((INT32U)pCdspReg2->cdspAwbSumRg2L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumRg2L[2]<<16)
		//	| ((INT32U)pCdspReg2->cdspAwbSumRg2L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumRg2L[0];
		//*sumrgh = (SINT32)(pCdspReg2->cdspAwbSumRg2H&0x07);//<<16)|((INT32U)pCdspReg2->cdspAwbSumRg2L[3]<<8)|((INT32U)pCdspReg2->cdspAwbSumRg2L[2]);
		*sumrgl = R_CDSP_SUM_RG2_L;
		*sumrgh = R_CDSP_SUM_RG2_H;
	} else if(section == 3) {
		//*sumrgl = ((INT32U)pCdspReg2->cdspAwbSumRg3L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumRg3L[2] << 16)
		//	|((INT32U)pCdspReg2->cdspAwbSumRg3L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumRg3L[0];
		//*sumrgh = (SINT32)(pCdspReg2->cdspAwbSumRg3H&0x07);//<<16)|((INT32U)pCdspReg2->cdspAwbSumRg3L[3]<<8)|((INT32U)pCdspReg2->cdspAwbSumRg3L[2]);
...
INT32S drv_l1_CdspGetAwbSumBG(INT8U section, INT32U *sumbgl, INT32S *sumbgh)
{
	...
	if(section == 1) {
		//*sumbgl = ((INT32U)pCdspReg2->cdspAwbSumBg1L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumBg1L[2] << 16)
		//	| ((INT32U)pCdspReg2->cdspAwbSumBg1L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumBg1L[0];
		//*sumbgh = (SINT32)(pCdspReg2->cdspAwbSumBg1H&0x07);//<<16)|;
		*sumbgl = R_CDSP_SUM_BG1_L;
		*sumbgh = R_CDSP_SUM_BG1_H;
	} else if(section == 2) {
		//*sumbgl = ((INT32U)pCdspReg2->cdspAwbSumBg2L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumBg2L[2] << 16)
		//	| ((INT32U)pCdspReg2->cdspAwbSumBg2L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumBg2L[0];
		//*sumbgh = (SINT32)(pCdspReg2->cdspAwbSumBg2H&0x07);//<<16)|
		*sumbgl = R_CDSP_SUM_BG2_L;
		*sumbgh = R_CDSP_SUM_BG2_H;
	} else if(section == 3) {
		//*sumbgl = ((INT32U)pCdspReg2->cdspAwbSumBg3L[3]<<24)|((INT32U)pCdspReg2->cdspAwbSumBg3L[2] << 16)
		//	| ((INT32U)pCdspReg2->cdspAwbSumBg3L[1]<<8)|(INT32U)pCdspReg2->cdspAwbSumBg3L[0];
...
{
	return R_CDSP_MD_DIFF_CNT;
}



//for Sharpen
void drv_l1_CdspSetSpEn(INT8U sp_en, INT8U uvf_en)
{
    R_CDSP_SP_CTRL &= ~(0x03);
    R_CDSP_SP_CTRL |= (0x1 << 31) | (sp_en & 0x01) | ((uvf_en & 0x01) << 1);

    //R_CDSP_HB_BLK_INTERVAL = 0x2ff;
}

INT8U drv_l1_CdspGetSpEn(void)
{
	return (R_CDSP_SP_CTRL  & 0x01);
}

INT8U drv_l1_CdspGetUVfEn(void)
{
	return (R_CDSP_SP_CTRL  & (0x01 << 1));
}

void drv_l1_CdspSetSpCtrl(INT8U mask, INT8U reduce, INT16U width)
{
    R_CDSP_SP_CTRL &= 0x03;

    R_CDSP_SP_CTRL |= ((mask & 0x01) << 2) | ((reduce & 0x0f) <<4) | ((width & 0x0fff) << 16);

	R_CDSP_SP_CTRL |= (0x1 << 31); //update next frame
}

void drv_l1_CdspSetSpImageSize(INT16U width)
{
    R_CDSP_SP_CTRL &= ~(0x0fff <<16) ;
    R_CDSP_SP_CTRL |= ((width & 0x0fff) << 16);

	R_CDSP_SP_CTRL |= (0x1 << 31); //update next frame
}


void drv_l1_CdspSetSpLevel(INT8U clip1, INT8U clip2, INT8U sharpenS, INT8U sharpenTh)
{
    R_CDSP_SP_THRD = clip1 | (clip2 << 8) | (sharpenS << 16) | (sharpenTh << 24);
}


void drv_l1_CdspSetSharpenS(INT8U sharpenS)
{
    R_CDSP_SP_THRD &= 0xff00ffff;
    R_CDSP_SP_THRD |= (sharpenS << 16);
...
void drv_l1_CdspSetSp_Table(INT32U *pSPTable)
{
	INT16U i;
	volatile INT32U *p_sram_hw = (volatile INT32U *) R_CDSP_MACRO_SDRAM_RW;

    //DBG_PRINT("\r\n\r\n drv_l1_CdspSetSp_Table = \r\n\r\n");
	R_CDSP_MACRO_CTRL = 0x1E1E;
	for(i=0; i<32; i++) {
        unsigned int val = *pSPTable++;
        *p_sram_hw++ = val;
...
//------------
//new new denoise(Bilateral Filter)
//for Denoise
void drv_l1_CdspSetHbNr_Table(INT32U *pNrTable)
{
	INT32U i;
	volatile INT32U *p_sram_hw = (volatile INT32U *) R_CDSP_MACRO_SDRAM_RW;

	R_CDSP_MACRO_CTRL = 0x1D1D;
	for(i=0; i<16; i++)
	{
		unsigned int val = *pNrTable++;
        *p_sram_hw++ = val;
    }
...
void drv_l1_CdspSetHbNrEn(INT8U nr_en)
{
  if (nr_en) {
    R_CDSP_HBNR_CTRL |= (1) | (1 << 31);
  }  else  {
    R_CDSP_HBNR_CTRL &= ~0x1;
    R_CDSP_HBNR_CTRL |= (1 << 31);
  }
}

INT8U drv_l1_CdspGetHbNrEn(void)
{
  return (R_CDSP_HBNR_CTRL & 0x01);
}

/**
* @brief	Bilateral Filter: LumaFilter, ChromaFilter Size
* @param	Luma Filter size= 0:bypass, 1:3x3, 2:5x7, 3:5x15
*@param     Chroma Filter size= 0:bypass, 1:3x3, 2:5x7, 3:5x15
* @return	none
 */
void drv_l1_CdspSetHbNrFilter_size(INT8U luma_size, INT8U chroma_size)
{
  R_CDSP_HBNR_CTRL &= ~(0xFF1);

  R_CDSP_HBNR_CTRL |= ((luma_size <<4) | (chroma_size << 8));
}

void drv_l1_CdspGetHbNrFilter_size(INT8U *pluma_size, INT8U *pchroma_size)
{
	*pluma_size = (R_CDSP_HBNR_CTRL >> 4) & 0x3;
	*pchroma_size = (R_CDSP_HBNR_CTRL >> 8)& 0x3;
}


/**
* @brief	Bilateral Filter: cut Y noise, If Y value < 16 will be cut Y value
* @param	1,2,3,4 indicate to cut 2,4,8,16 from Y value
* @return	none
 */
void drv_l1_CdspSetHbNrYnoise_clip(INT16U clip_y_val)
{
  R_CDSP_HBNR_CTRL &= ~(0x7 << 12);
  ...
  R_CDSP_HBNR_CTRL |= clip_y_val << 12;
}

INT8U drv_l1_CdspGetHbNrYnoise_clip(void)
{
  return ((R_CDSP_HBNR_CTRL >>12) & 0x7);
}

/**
* @brief: Edge energy threshold of 2 Sobel-like HPFs
* @param: [10-bit (1~1023),def=32]
* @return: none
 */
void drv_l1_CdspSetHbNrEdge_thr(INT16U edge_t)
{
  R_CDSP_HBNR_CTRL &= ~(0x3FF << 16);

  R_CDSP_HBNR_CTRL |= edge_t << 20;
}

INT16U drv_l1_CdspGetHbNrEdge_thr(void)
{
  return ((R_CDSP_HBNR_CTRL >> 20) & 0x3FF);
}

/**
* @brief: Enable Anti-Jaggy & Lower limit of edge detail energy threshold
* @param: [1-bit (0~1), def=1],[10-bit (1~1023),def=16]
* @return: none
 */
void drv_l1_CdspSetHbNrAJ_en(INT8U aj_en)
{
  R_CDSP_HBAJ_CTRL &= ~(0x1 << 12);

  R_CDSP_HBAJ_CTRL |= (aj_en & 0x1);
}

void drv_l1_CdspSetHbNrAJ_en_thr(INT8U aj_en, INT16U ajedge_t)
{
  R_CDSP_HBAJ_CTRL &= ~0x1FFF;

  R_CDSP_HBAJ_CTRL |= (ajedge_t & 0x3FF) | (aj_en & 0x1);
}

INT16U drv_l1_CdspGetHbNrAJ_thr(void)
{
  return (R_CDSP_HBAJ_CTRL & 0x3FF);
}

INT8U drv_l1_CdspGetHbNrAJ_en(void)
{
  return ((R_CDSP_HBAJ_CTRL >> 12) & 0x1);
}
...
// WDR
////////////////////////////////////////////////////////////////////////
void drv_l1_CdspSetWDREn(INT8U wdr_en)
{
 //DBG_PRINT("*** wdr_en = %d, R_CDSP_WDR_CTRL = 0x%x ***\r\n", wdr_en, R_CDSP_WDR_CTRL);
    /*b[8]:不開啟會造成上方多一條線,b[16][12]:不是主要影響上方多一條線的因素*/

    if (wdr_en) {
        //R_CDSP_WDR_CTRL |= 0x1;
        R_CDSP_WDR_CTRL |= 0x11101;
    }  else  {
        //R_CDSP_WDR_CTRL &= ~0x1;
        R_CDSP_WDR_CTRL &= 0x11106;
    }

 // DBG_PRINT("*** Out: R_CDSP_WDR_CTRL = 0x%x ***\r\n", R_CDSP_WDR_CTRL);
}



INT8U drv_l1_CdspGetWDREn(void)
{
  return (R_CDSP_WDR_CTRL & 0x01);
}

/**
 * @brief	Wdr UV referance LUT change
 * @param	uv_en[in]: enable or disable
 * @return	none
 */

void drv_l1_CdspSetWDRCtrl(void)
{
    R_CDSP_WDR_CTRL = 0x11007;
}

void drv_l1_CdspSetWDRUVEn(INT8U uv_en)
{
    unsigned int tmp = R_CDSP_WDR_CTRL;

    //DBG_PRINT("1: uv_en = %d, R_CDSP_WDR_CTRL = 0x%x, 0x%x\r\n", uv_en, R_CDSP_WDR_CTRL, tmp);

  //R_CDSP_WDR_CTRL &= 0xffffEfff;
  if (uv_en) {
    R_CDSP_WDR_CTRL |= (0x1 << 1);
  }  else  {
  ...
  //DBG_PRINT("2R_CDSP_WDR_CTRL = 0x%x, 0x%x\r\n", R_CDSP_WDR_CTRL, tmp);

  //R_CDSP_WDR_CTRL |= (1 << 12);
}

void drv_l1_CdspSetWDRShadowEn(INT8U en)
{
    if(en == 1)
    {
        R_CDSP_WDR_CTRL |= (1 << 12);
...
INT8U drv_l1_CdspGetWDRUVEn(void)
{
  return ((R_CDSP_WDR_CTRL >> 1) & 0x01);
}

void drv_l1_CdspSetWDR_SOF_RST(INT8U rst)
{
	if (rst == 1) {
		R_CDSP_WDR_CTRL |= (0x1 << 8);
	}  else
	{
		R_CDSP_WDR_CTRL &= ~(0x1 << 8);
	}
...
INT8U drv_l1_CdspGetWDR_SOF_RST(void)
{
	return R_CDSP_WDR_CTRL & (0x1 << 8);
}

void drv_l1_CdspSetWDRUVLevl(INT8U chro_fact)
{
    R_CDSP_WDR_PARA &= 0xff00ffff;
    R_CDSP_WDR_PARA |= ((chro_fact & 0xff) << 16);
}

INT8U drv_l1_CdspGetWDRUVLevl(void)
{
  return ((R_CDSP_WDR_PARA >> 16) & 0xFF);
}


/**
 * @brief	Boundary referance mode
* @param	0: Duplicate , 1: Mirror image valid data
 * @return	none
 */
void drv_l1_CdspSetWDRBndMod(INT8U bnd_case)
{
  if (bnd_case) {
    R_CDSP_WDR_CTRL |= (0x1 << 2);
  }  else  {
    R_CDSP_WDR_CTRL &= ~(0x1 << 2);
  }
...
/**
 * @brief	Blanking out time between last two lines.
* @param	0~ 0xFFF
 * @return	none
 */
void drv_l1_CdspSetWDRLHcnt(INT16U time)
{
  if(time > 0xFFF) time = 0xFFF;
  R_CDSP_WDR_LHCNT = time;
}


INT8U drv_l1_CdspGetWDRBndMod(void)
{
  return ((R_CDSP_WDR_CTRL >> 2)& 0x01);
}

/**
* @brief	WDR Windows size
* @param	If win_bg = 1x1, win_i only can set 1x1
*@param         win_i: 1x1 set 0 , 3x3 set 1 (simulator 1x1 set 1, 3x3 set 3 )
*@param         win_bg: 1x1,3x3, 3x9 set 0,1,2 (simulator set 1,3,5 )
* @return	none
 */
void drv_l1_CdspSetWDRWinPix_Bckgd(INT8U win_i, INT8U win_bg)
{
  win_i &= 0x01;
  win_bg &= 0x03;
  ...
  R_CDSP_WDR_PARA |= (win_i | (win_bg << 2));

}

void drv_l1_CdspGetWDRWinPix_Bckgd(INT8U *pwin_i, INT8U *pwin_bg)
{
	*pwin_i = R_CDSP_WDR_PARA & 0x01;
	*pwin_bg = (R_CDSP_WDR_PARA >> 2)& 0x3;
}

//++++++ Write Table & Read Table
//WDR Table 256 bytes
void drv_l1_CdspSetWDR_Table(INT32U *pWDRTable, INT32U table_sel)
{
	INT32U i;

    //DBG_PRINT("\r\n\r\ndrv_l1_CdspSetWDR_Table[%d]=", table_sel);
	for(i=0; i<64; i++) {
        unsigned int val;
		val = *pWDRTable++;
		R_CDSP_WDR_LUT_TBL = val;
	...
	if(table_sel == 0)
        R_CDSP_WDRLUT_CTRL = 0x0;
    else
        R_CDSP_WDRLUT_CTRL = 0x1;
}

void drv_l1_CdspRead_WDR_Table(
	INT32U *sp_val,
	INT32U table_sel
)
{
	...
	if(table_sel == 0)
        ...
        R_CDSP_WDRLUT_CTRL = 0x2;

	p_sp_val = sp_val;
	p_sram_hw = &R_CDSP_WDR_LUT_TBL;

	DBG_PRINT("\r\n\r\ndrv_l1_CdspRead_WDR_Table[%d]=", table_sel);
        for(i = 0 ; i < 64 ; i++)
	{
          unsigned int val;
          val = *p_sram_hw;
...
// Post Lut
////////////////////////////////////////////////////////////////////////
void drv_l1_CdspSet_PostLut(INT32U *p_rgblut)
{
	INT32U i;
	volatile INT32U *p_sram_hw;

	R_CDSP_MACRO_CTRL = 0x0D0D;

    p_sram_hw = (INT32U *)CDSP_TABLE_BASEADDR;
	for(i=0; i<192; i++)    /*32*64*3=256*8*3*/
	{
		INT32U val = *p_rgblut++;
		*p_sram_hw++ = val;
    }
...
void drv_l1_CdspRead_PostLut(INT32U *table_val)
{
	INT32U i;
    INT32U reg_tmp;
	INT32U *p_tb_val;
...
void drv_l1_CdspSetPostLutEn(INT8U en)
{
    R_CDSP_LUTRGB_EN = en;
}

/**
 * @brief	cdsp set YUV Pscaler path
 * @param	onPscale[in]: 0:Off Pscaler path, 1:Open Pscaler path
 * @return	none
 */
void drv_l1_CdspSetYuvPscalePath(INT8U onPscale)
{
	if(onPscale) {
		R_CDSP_DO |= (0x1 << 7);	//Open Pacler path
	} else {
		R_CDSP_DO &= ~(0x1 << 7);	//Off
	}
}
...
INT32U drv_l1_cdsp_fifo_index_get(void)
{
	return ((R_CDSP_DMA_COFG & 0x100)>>8);
}


void drv_l1_CdspSetAddr(INT32U work_buffer)
{
	R_CDSP_AE_WIN_ABUFADDR = work_buffer;
	R_CDSP_AE_WIN_BBUFADDR = work_buffer;
	R_CDSP_DMA_YUVABUF_SADDR = work_buffer;
	R_CDSP_DMA_YUVBBUF_SADDR = work_buffer;
	R_CDSP_DMA_RAWBUF_SADDR = work_buffer;
	R_CDSP_MD_DMA_SADDR = work_buffer;
}
