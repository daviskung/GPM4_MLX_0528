...
INT32S jpeg_simple_decode(void)
{
    ...
    image_source.type_ID.FileHandle = fs_open((char *)file_info.f_name, O_RDONLY);
	...
	while (1) {
         ...
         if (decode_state == IMAGE_CODEC_DECODE_END) {
			 fs_close(image_source.type_ID.FileHandle);
			 break;
...
INT32S jpeg_simple_once_encode(INT32U input_buf)
{
	...
	if (DISKUSED == FS_SD1){
		...
		sprintf((char *)path, (const char *)"K:\\image_encode.jpeg");
	}
	else if (DISKUSED == FS_NAND1){
