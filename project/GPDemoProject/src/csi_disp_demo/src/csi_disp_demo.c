...
#define SP_num_CHR_SIZE                     (SP_unit_H_SIZE*SP_unit_V_SIZE*SP_SIZE_CH)
...
#define SP_sm_num_CHR_SIZE                  (SP_sm_unit_H_SIZE*SP_sm_unit_V_SIZE*SP_SIZE_CH)
...
static void disp_task_entry(void const *parm)
{
	...
	// PPU init End ***
		...
		userDefine_spNum = 0;
		for(lpcnt = 0 ; lpcnt < 4 ; lpcnt++) // 32x64 Big Font(line1)
        {
			set_sprite_init(userDefine_spNum,(INT32U)&Sprite023_half_SP);

			if(DISPLAY_DEVICE == DISDEV_TFT)
				{
				x_pos=5*SP_unit_H_SIZE + (lpcnt%5)*(SP_unit_H_SIZE);
				y_pos=(lpcnt/5)*SP_unit_V_SIZE;
				}
		...
		for(lpcnt = 5 ; lpcnt < 9 ; lpcnt++)	// 16x32 small Font  (line2)
		{
			set_sprite_init(userDefine_spNum,(INT32U)&Sprite025_half_C_SP);
			if(DISPLAY_DEVICE == DISDEV_TFT)
				{
				x_pos=lpcnt*(SP_sm_unit_H_SIZE) + 9*SP_sm_unit_H_SIZE;
	...
	if ((pMLX_TH32x24_Para->MLX_TH32x24_SENSOR_EXT_STATUS == SENSOR_CONNECT) &&
		...
		*/
			...
			//
			//	32x64 Big Font  (line1)
			//
			
			sprite_base_addr = (INT32U)_SP_32x64_newNUM_CellData;
			if ((TmpDispBuf/1000) != 0)
				{
				sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +		// 千位數 
				( ((TmpDispBuf/1000)+1) * SP_num_CHR_SIZE));
				}
			else
				{
			...
			sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +		// 百位數 
				( ((TmpDispBuf/100)%10 + 1) * SP_num_CHR_SIZE));
            ...
            for(i=0;i<sp_ptr.nSP_CharNum;i++)
            {
			 ...
			 sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +  // 十位數 
			 	(((TmpDispBuf/10)%10 + 1 ) * SP_num_CHR_SIZE));
			...
			sprite_base_addr = (INT32U)_SP_32x64_number_p_CellData;
			 sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +  // 個位數 
			 	((TmpDispBuf%10 + 1) * SP_num_CHR_SIZE));
			 	//((pMLX_TH32x24_Para->MLX_TH32x24_TmpMax%10) * SP_CHR_SIZE));
			...
			sprite_base_addr = (INT32U)_SP_32x64_unit_CellData;
			...
			sprite_base_addr = (INT32U)_SP_16x32_number_CellData;
			if ((AlertTmpSet_tmp/1000) != 0)
				{
				sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +		// 千位數 
				( ((AlertTmpSet_tmp/1000)+1) * SP_sm_num_CHR_SIZE));
				}
			else
				{
				sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +	0);	// 千位數  = Null
			...
			sprite_base_addr = (INT32U)_SP_16x32_number_CellData;
			sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +	// 百位數 
			((AlertTmpSet_tmp/100)%10 + 1)  * SP_sm_num_CHR_SIZE);
			...
			sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +	// 十位數 
				( ((AlertTmpSet_tmp/10)%10 + 1)* SP_sm_num_CHR_SIZE)); //
            ...
            for(i=0;i<sp_ptr.nSP_CharNum;i++)
            {
			 ...
			 sprite_base_addr = (INT32U)_SP_16x32_number_p_CellData;
			 sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +  // 個位數 
			 	( ((AlertTmpSet_tmp%10)+1) * SP_sm_num_CHR_SIZE)); // fortest
			...
			sprite_base_addr = (INT32U)_SP_16x32_unit_CellData;
			if (pMLX_TH32x24_Para->MLX_TH32x24_TMPunit_SET == 0)
				{
				sprite_characterNum_pos_addr = (INT32U)(sprite_base_addr +  // unit half C
...
static void prcess_task_entry(void const *parm)
{
	...
	if(avg_buf_Enable ==0){ 	// [0] ~ [3] <- new data
		...
		DBG_PRINT(" CalculateTo [t=%d] %d \r\n",xTaskGetTickCount()-TimeCnt1,pMLX_TH32x24_Para->MLX_TH32x24_TmpMax);
