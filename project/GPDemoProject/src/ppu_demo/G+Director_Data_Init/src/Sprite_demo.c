...
#define SP_SIGNED_POS_ENABLE         1
#define SP_NEW_EXTEND_RAM_ENABLE     1
#define SP_HARDWARE_POS_GET_ENABLE   1

// Define Max sprite ram size and buffer size
#define Spritebuf_number             1024
#define Sprite_ptr_number            1024
#define SpriteRAM_number             1024
#define SpriteExRAM_number           1024
...
SPRITE	Spritebuf[Spritebuf_number];
SpN_ptr Sprite_ptr[Sprite_ptr_number];
SpN_RAM	SpriteRAM[SpriteRAM_number];
...
#if SP_NEW_EXTEND_RAM_ENABLE == 1
SpN_EX_RAM SpriteExRAM[SpriteExRAM_number];
#endif

// Gobel value
static PPU_SCREEN_STRUCT ppu_hardware_screen_range = {-512, 511, 511, -512};
static INT32U nV_Size,nH_Size,free_h_size = 0,free_v_size = 0;
INT32U ExSpritebuf,Sprite_ex_ptr,ExSpriteRAM,EXSP_NUM;

INT32S sprite_screen_size_get(Sprite_Coordinate_Mode display_mode, INT32S *Width, INT32S *Height, PPU_SCREEN_STRUCT *screen_ptr);

static INT32S ppu_memset(INT8S *dest,INT8U byte, INT32U Len)
{
	INT32S i = 0;
...
// Sprite API
void set_sprite_free_size(INT32U h_size,INT32U v_size)
{
    free_h_size = h_size;
    free_v_size = v_size;
}
			...
			if (scroll)
			{
                switch(display_mode)
                {
                    ...
                    case Sprite_Coordinate_1024X768:
		                 ...
		                 screen_height = 768;
                         break;
                    case Sprite_Coordinate_Freemode:
       		             if(free_h_size && free_v_size)
       		             {
                            screen_width = free_h_size;
		                    screen_height = free_v_size;
       		             }
       		             else
       		             {
                            screen_width = 640;
...
void paint_ppu_spriteram(PPU_REGISTER_SETS *p_register_set,Sprite_Coordinate_Mode display_mode, Coordinate_Change_Mode coordinate_mode, INT16U sprite_number)
{
   ...
   PSpN_ptr  sp_ptr;
   PPU_SCREEN_STRUCT screen_range;

#if SP_HARDWARE_POS_GET_ENABLE == 1
    sprite_screen_size_get(display_mode, &nScrWidth, &nScrHeight, &screen_range);
#else
    switch(display_mode)
    {
       case Sprite_Coordinate_320X240:
      ...
      case Sprite_Coordinate_Freemode:
            if(free_h_size && free_v_size)
            {
                nScrWidth = free_h_size;
                nScrHeight = free_v_size;
            }
            else
            {
                nScrWidth = 640;
    ...
    if((nScrWidth != 0) && (nScrHeight != 0) && p_register_set)
    {
        ...
        ppu_memset((INT8U *)sprite_attr,0,SpriteRAM_number*sizeof(SpN_RAM));
        nSprite=(PSPRITE)Spritebuf;
        sp_ptr=(PSpN_ptr)Sprite_ptr;
        //PPU_MemSet ((INT32U *)sp_ptr,0,Sprite_ptr_number*sizeof(PSpN_ptr),32);
        ppu_memset((INT8U *)sp_ptr,0,Sprite_ptr_number*sizeof(PSpN_ptr));
    #if SP_NEW_EXTEND_RAM_ENABLE == 1
        sprite_ex_attr=(PSpN_EX_RAM)SpriteExRAM;
        //PPU_MemSet ((INT32U *)sprite_ex_attr,0,SpriteExRAM_number*sizeof(SpN_EX_RAM),32);
        ppu_memset((INT8U *)sprite_ex_attr,0,SpriteExRAM_number*sizeof(SpN_EX_RAM));
    #endif
        cache_invalid_range((INT32U)&SpriteRAM[0],SpriteRAM_number*sizeof(SpN_RAM));
        cache_invalid_range((INT32U)&SpriteExRAM[0],SpriteRAM_number*sizeof(SpN_EX_RAM));
              ...
              for(j=0; j<max_sp_combination_number; j++)
              {
                  ...
                  else if (coordinate_mode == LeftTop2Center_coordinate)
                  {
                     // LeftTop2Center_coordinate
                         ...
                         nPosYN = (screen_range.screen_yt) - 1 - nSprite->nPosY - temp_y - (nV_Size>>1);
                     #else
                         nPosXN = nSprite->nPosX + temp_x - (nScrWidth>>1) + (nH_Size>>1) + 1;
                         nPosYN = (nScrHeight>>1) - 1 - nSprite->nPosY - temp_y - (nV_Size>>1);
                     #endif
...
void load_sprite_bin(INT16U display_sp_num,INT16U sp_num,INT16S sp_pos_x,INT16S sp_pos_y,INT32U* SPRITE_ATX_BIN)
{
    ...
    temp = (INT8U*)&(sprite_pointer->SpCell);
    sprite_offset = temp[0] + (temp[1] << 8) + (temp[2] << 16) + (temp[3] << 24);
            ...
            for(i=0;i<sprite_number;i++) {// Check sprite number?
               if(nSprite->SpCell != 0) { // Check sprite if exist?
                  ...
                  for(j=0; j<max_sp_combination_number; j++)
                  {
                      ...
                      sprite_cdm_attr++;
                      #if SP_STRUCTURE_CMDDIS_EN == 1//((MCU_VERSION >= GPL326XX))
               ...
               if(nSprite->SpCell!= 0) { // Check sprite if exist?
                  ...
                  for(j=0; j<max_sp_combination_number; j++)
                  {
                      ...
                      sprite_cdm_attr++;
                         ...
                         sprite_cdm_attr->uPosX_16 |= sp_no_cdm_en; // Disable CMD mode
                      #endif
                      sprite_cdm_attr++;
                  }
               }
...
When the 1024x768 mode is selected, the sprite coordinate cannot be out of the screen at X-axis.
...
INT32S sprite_screen_size_get(Sprite_Coordinate_Mode display_mode, INT32S *Width, INT32S *Height, PPU_SCREEN_STRUCT *screen_ptr)
{
    ...
    if((Width == 0) || (Height == 0) || (screen_ptr == 0))
        return -1;
    ...
    switch(display_mode)
    {
       case Sprite_Coordinate_320X240:
            *Width = 320;
            *Height = 240;
            screen_ptr->screen_xl = -160;
            screen_ptr->screen_xr = 159;
            screen_ptr->screen_yt = 127;
            screen_ptr->screen_yb = -112;
            break;

       case Sprite_Coordinate_640X480:
            *Width = 640;
            *Height = 480;
            screen_ptr->screen_xl = -320;
            screen_ptr->screen_xr = 319;
            screen_ptr->screen_yt = 255;
            screen_ptr->screen_yb = -224;
            break;

       case Sprite_Coordinate_480X234:
            *Width = 480;
            *Height = 234;
            screen_ptr->screen_xl = -240;
            screen_ptr->screen_xr = 239;
            screen_ptr->screen_yt = 127;
            screen_ptr->screen_yb = -106;
            break;
       ...
       case Sprite_Coordinate_480X272:
            ...
            *Height = 272;
            ...
            screen_ptr->screen_xr = 239;
            screen_ptr->screen_yt = 127;
            screen_ptr->screen_yb = -144;
            break;

       case Sprite_Coordinate_720X480:
            *Width = 720;
            *Height = 480;
            screen_ptr->screen_xl = -360;
            screen_ptr->screen_xr = 359;
            screen_ptr->screen_yt = 255;
            screen_ptr->screen_yb = -224;
            break;
       ...
       case Sprite_Coordinate_800X480:
            *Width = 800;
            *Height = 480;
            screen_ptr->screen_xl = -400;
            screen_ptr->screen_xr = 399;
            screen_ptr->screen_yt = 255;
            screen_ptr->screen_yb = -224;
            break;
       ...
       case Sprite_Coordinate_800X600:
            ...
            *Height = 600;
            screen_ptr->screen_xl = -400;
            screen_ptr->screen_xr = 399;
            screen_ptr->screen_yt = 299;
            screen_ptr->screen_yb = -300;
            break;

       case Sprite_Coordinate_1024X768:
            *Width = 1024;
            *Height = 768;
            screen_ptr->screen_xl = -512;
            screen_ptr->screen_xr = 511;
            screen_ptr->screen_yt = 383;
            screen_ptr->screen_yb = -384;
            break;
       ...
       case Sprite_Coordinate_Freemode:
            if(free_h_size && free_v_size)
            {
                *Width = free_h_size;
                *Height = free_v_size;
                screen_ptr->screen_xl = (-1*(free_h_size/2));
                screen_ptr->screen_xr = ((free_h_size/2)-1);
                if(free_v_size <= 272)
                {
                    screen_ptr->screen_yt = 127;
                    screen_ptr->screen_yb = (128-free_v_size);
                }
                else if(free_v_size <= 576)
                {
                    screen_ptr->screen_yt = 255;
                    screen_ptr->screen_yb = (256-free_v_size);
                }
                else if(free_v_size <= 1024)
                {
                    screen_ptr->screen_yt = ((free_v_size/2)-1);
                    screen_ptr->screen_yb = (-1*(free_v_size/2));
                }
                else
                {
                    *Width = 320;
...
void spriteV3D_Pos_Coordinate_Change(Sprite_Coordinate_Mode display_mode,Coordinate_Change_Mode coordinate_mode,POS_STRUCT_PTR in)
{
    ...
    if(coordinate_mode == LeftTop2Center_coordinate)
    {
    #if SP_HARDWARE_POS_GET_ENABLE == 1
        sprite_screen_size_get(display_mode, &nScrWidth, &nScrHeight, &screen_range);
	    if((nScrWidth != 0) && (nScrHeight != 0) && (in != 0))
	    {
            Sprite_pos = in;
            ...
            else if(coordinate_mode == LeftTop2Center_coordinate)
            {
                ...
                nPosYN = screen_range.screen_yt - temp_y;
            }
...
#if 1
void SpCellXPos25d(INT32U xnum, INT32U ynum, FP32 xpos0, FP32 xpos1, FP32 xpos2, FP32 xpos3)
#else
...
#if 1
void SpCellYPos25d(INT32U xnum, INT32U ynum, FP32 ypos0, FP32 ypos1, FP32 ypos2, FP32 ypos3)
#else
...
void exsp_group_set(INT32U ex_ram,INT8U group_id)
{
    ...
    sprite_cdm_attr = (PSpN_RAM)ex_ram;
    sprite_cdm_attr++;
    sprite_cdm_attr->uPosX_16 |=((group_id & mask_group_id)<<b_exspn_group);
#else
    if(EXSP_CMD_COMPARE())
    {
        sprite_cdm_attr = (PSpN_RAM)ex_ram;
...
{
    INT32S nPosXN,nPosYN,INTS;
#if SP_SIGNED_POS_ENABLE == 1
    INT32S INT;
#else
    INT32U INT;
#endif
    INT32U i,k,h,nSP_Hsize,nSP_Vsize,sp_num_addr;
    INT32U temp_x,temp_y,temp_ck;
    PSPRITE nSprite;
    PSpN16_CellIdx pSpCell;
...
#if SP_NEW_EXTEND_RAM_ENABLE == 1//((MCU_VERSION >= GPL326XX))
	...
	if(sp_ptr->nSP_CharNum > 1)
    {
		...
		for(i=0;i<sp_ptr->nSP_CharNum;i++)
		{
			...
			INT = (INT32S)SUM-nPosXN;
			INT = (INT32S)INT+nPosXN;
			Sprite_pos.x1 = INT;
			//x2
			...
			INT = (INT32S)SUM-nPosYN;
			INT = (INT32S)INT+nPosYN;
			Sprite_pos.y1 = INT;
			//y2
	...
	else
	{
	    ...
	    #endif
	 #endif
        #if 0//SP_NEW_EXTEND_RAM_ENABLE == 1
            sp_interpolation_enable(sprite_ex_attr);
        #endif
	    spriteV3D_Pos_Coordinate_Change(display_mode,coordinate_mode,(POS_STRUCT_PTR)&Sprite_pos);
	    gplib_ppu_sprite_attribute_25d_position_set((SpN_RAM *)sp_num_addr,(POS_STRUCT_PTR)&Sprite_pos);
...
void sprite25D_Pos_Coordinate_Chang(Sprite_Coordinate_Mode display_mode,Coordinate_Change_Mode coordinate_mode,INT32U sprite_H,INT32U sprite_V,POS_STRUCT_PTR in)
{
    INT32S nPosXN,nPosYN,temp_x,temp_y,nScrWidth,nScrHeight;
    POS_STRUCT_PTR Sprite_pos;
    ...
    temp_y=Sprite_pos->y0;
    if(coordinate_mode == Center2LeftTop_coordinate){
      nPosXN = temp_x + 0 - (nScrWidth>>1) + (sprite_H>>1) + 1;
      nPosYN = (nScrHeight>>1) - 1 - temp_y + 0 - (sprite_V>>1);
    }else if(coordinate_mode == LeftTop2Center_coordinate){
      nPosXN = temp_x + 0 - (nScrWidth>>1) + (sprite_H>>1) + 1;
      nPosYN = (nScrHeight>>1) - 1 - temp_y - 0 - (sprite_V>>1);
    }
